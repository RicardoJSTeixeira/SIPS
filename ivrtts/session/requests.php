...
switch($zero) {
    case 'Login' : Login($email, $password); break;
...
function Login($email, $password) {
    ...
    $params = array($email);
    $results = $db->rawQuery("SELECT id_user, password, salt FROM zero.users WHERE email = ? LIMIT 1", $params);
    ...
    if(count($results) !== 0){
        ...
        if($password === $results[0]['password']){
            ...
            $db->rawInsert("INSERT INTO zero.user_auth_log (id_user, ip, event, event_time) VALUES (?, INET_ATON(?), ?, ?)", $params);
        ...
        $db->rawInsert("INSERT INTO zero.login_attempts (ip, event_time) VALUES (INET_ATON(?), ?)", $params);
