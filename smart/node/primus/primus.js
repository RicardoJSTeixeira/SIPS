 ...
 * @api private
 */
function Primus(url, port, options) {
  ...
  this.port = port;                // Parse the URL to a readable format.
...
Primus.prototype.reconnect = function reconnect(callback, opts) {
  ...
  opts.maxDelay = opts.maxDelay || 60000;  // Maximum delay.
  opts.minDelay = opts.minDelay || 1000;       // Minimum delay.
  opts.retries = opts.retries || 5;          // Amount of allowed retries.
...
Primus.prototype.uri = function uri(protocol, querystring) {
  ...
  server.push(this.url.auth ? this.url.auth + '@' + this.url.host : this.url.host + ':' + this.port, this.pathname.slice(1));
