...
$(document).ready(function() {
    ...
    /*   $('#breaks-show-btn').click(function(e) {
    ...
    $('#logout a').click(function(e) {
        ...
        // ask verification
        $.SmartMessageBox({
            title: "<i class='fa fa-sign-out txt-color-orangeDark'></i> Logout <span class='txt-color-orangeDark'><strong>" + firstname + " " + lastname + "</strong></span> ?",
            content: "You can improve your security further after logging out by closing this opened browser",
            buttons: '[No][Yes]'

        }, function(ButtonPressed) {
            if (ButtonPressed == "Yes") {
                $.post('/php/action_user.php',{action:'writeLog', user:user, domain:domain, actions:'Logout',target:'Main', date:moment().format('YYYY-MM-DD HH:mm:ss')}, function(data){},'json');
...
function nav_page_height() {
    ...
    if (setHeight > windowHeight) {// if content height exceedes actual window height and menuHeight
        ...
        $.root_.css('min-height', setHeight + 49 + 'px');
        //new line
...
function loadURL(url, container) {
    ...
    $.ajax({
        ...
        success: function(data) {
            if ($('#content').attr('channel-loaded') === 'voice') {

                $('#content').attr('channel-loaded', '');
                $('#voice-workspace').addClass('hidden');
                $('#voice-channel-control').addClass('hidden');
                container.removeClass('hidden').hide().html('<h1><i class="fa fa-cog fa-spin"></i> Loading...</h1>').load(url).fadeIn('slow');
        ...
        error: function(xhr, ajaxOptions, thrownError) {
            container.removeClass('hidden').html('<h4 style="margin-top:10px; display:block; text-align:left"><i class="fa fa-warning txt-color-orangeDark"></i> Error 404! Page not found.</h4>');
