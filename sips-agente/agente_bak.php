...
function JanelaNovoCliente()
	{
        ...
        function nc_log(){
            $.post("/client_files/acusticamedica/novocliente/nc_log.php",{inOUT:inOUT_hack,agent_log_id:agent_log_id,lead_id:nc_live_id},function(data){console.log(data);});
        }
// ################################################################################
...
// Send the Manual Dial Next Number request
		...
		if (document.vicidial_form.LeadPreview.checked==true)
			...
			var man_preview = 'YES';
			var man_status = "<a href=\"#\" onclick=\"ManualDialOnly('" + manual_dial_only_type_flag + "')\" style='font-size:14px; font-weight: bold; text-decoration:underline'>Marcar Principal</a> ou <a href=\"#\" onclick=\"ManualDialOnly('ALTPhonE')\" style='font-size:14px; font-weight: bold; text-decoration:underline'>Marcar Alternativo</a> ou <a href=\"#\" onclick=\"ManualDialOnly('AddresS3')\" style='font-size:14px; font-weight: bold; text-decoration:underline'>Marcar Alternativo 2</a> ou <a href=\"#\" onclick=\"ManualDialSkip()\" style='font-size:14px; font-weight: bold; text-decoration:underline'>Cancelar Ligação</a>"; 
...
$("#DispoMinimizeButton").live("click", function(){
	
	hideDiv('DispoSelectBox');
	$("#div-dispo-control-img").css("visibility", "visible");
	$("#div-dispo-control-text").html("Feedbacks");

})

$("#div-dispo-control").live("click", function(){

	if($("#div-dispo-control-text").html() !== ""){
	showDiv('DispoSelectBox');
	$("#div-dispo-control-img").css("visibility", "hidden");
	$("#div-dispo-control-text").html("");
	}
	...
	function DispoSelect_submit()
		...
		if($("#vcFormIFrame").contents().find(".radiocheck-required:checked").length){
			...
			if($("#vcFormIFrame").contents().find(".radiocheck-required:checked").val() === "CATOS"){
					if($("#vcFormIFrame").contents().find(".multiple-requiredconsultorio option:selected").val() === "nenhum" || typeof $("#vcFormIFrame").contents().find(".multiple-requiredconsultorio option:selected").html() === "undefined" ) {alert_box("Campo do CATO com erros de preenchimento ou vazio."); $("#DispoMinimizeButton").click();  return false;}
			}
			if($("#vcFormIFrame").contents().find(".radiocheck-required:checked").val() === "Branch"){
					if($("#vcFormIFrame").contents().find(".multiple-requiredconsultoriodois option:selected").val() === "nenhum" || typeof $("#vcFormIFrame").contents().find(".multiple-requiredconsultoriodois option:selected").html() === "undefined" ) {alert_box("Campo do Consultório com erros de preenchimento ou vazio."); $("#DispoMinimizeButton").click();  return false;}
			}
			
			 } else { alert_box("Campo do tipo de consulta com erros de preenchimento."); $("#DispoMinimizeButton").click();  return false; }
		

		
		
		if($("#vcFormIFrame").contents().find(".hour-required").val().length === "---" ){ alert_box("Campo da Hora da Marcação com erros de preenchimento ou vazio."); $("#DispoMinimizeButton").click();  return false; }
		if($("#vcFormIFrame").contents().find(".minute-required").val().length === "---" ){ alert_box("Campo dos Minutos da Marcação com erros de preenchimento ou vazio."); $("#DispoMinimizeButton").click();  return false; }
		if($("#vcFormIFrame").contents().find(".date-required").val().length <= 0 ){ alert_box("Campo da Data da Marcação com erros de preenchimento ou vazio."); $("#DispoMinimizeButton").click();  return false; }
...
// Generate the Closer In Group Chooser panel
	function CloserSelectContent_create()
		...
		if ( (VU_agent_choose_ingroups == '1') && (manager_ingroups_set < 1) )
			{
            var live_CSC_HTML = "<table cellpadding=\"5\" cellspacing=\"5\" width=\"500px\"><tr><td><b>GROUPS NOT SELECTED</b></td><td><b>SELECTED GROUPS</b></td></tr><tr><td  height=\"300px\" width=\"240px\" valign=\"top\"><span id=CloserSelectAdd> &nbsp; <a href=\"#\" onclick=\"CloserSelect_change('-----ADD-ALL-----','ADD');return false;\"><b>--- ADD ALL ---</b><br />";
			var loop_ct = 0;
                            ...
                            <table border=0>
                                <tr>
                                    <td style='min-width:122px'>
                                        <span style="cursor: pointer;" onClick="mpass('<?php echo $user; ?>')"><img src='/images/icons/virus_protection.png' style="vertical-align: middle" /> Alterar Password  </span>
                                    </td>
                                    <td style='min-width:122px'>
                                        <span style="cursor: pointer;" onClick="ultimoscontactos('<?php echo $user; ?>')"><img src='/images/icons/telephone_go.png' style="vertical-align: middle" /> Últimas Chamadas  </span>
                                    </td>
                                    <td>
...
<!-------------------------------------------------------------------------------------------------------------------------------------------->
		...
		<tr id="ResumeControl" <? if ($auto_dial_level < 1) { echo "style='display:none'"; } ?>>
			<td height='35px'><img src='/images/icons/control_play_blue.png' /></td>
			<td height='35px'>
				<a href="#" onClick="vdadready">Retomar Chamadas</a>
			</td>
		</tr>
		<tr id="PauseControl" <? if ($auto_dial_level < 1) { echo "style='display:none'"; } ?>>
		...
		<tr id='search' onClick="OpeNSearcHForMDisplaYBox();" style='cursor:pointer'>
			<td height='35px'><img src='/images/icons/zoom_32.png' /></td>
			<td height='35px'>
				Procurar Contactos
			</td>
		</tr>
		<tr id='mdial' onClick="NeWManuaLDiaLCalL('NO');return false;" style='cursor:pointer'>
		...
		<div id='tpausa' style='border-top:2px solid #e8edff;'>
		<table>
			<tr>
...
<td>
<div class=cc-mstyle onClick="Dinline('MainPanel'); Dnone('FormPanel'); Dnone('sips-crm');" style='cursor:pointer; width:60%; float:left;'> 
<table> 
<tr>
...
<td>
<div id='div-dispo-control' class=cc-mstyle style='cursor:pointer; width:60%; float:right;'> 
<table> 
<tr>
<td style='width:48px;'><img id='div-dispo-control-img' style='float:right; visibility:hidden' src='/images/icons/document_prepare_32.png' /></td>
<td id='div-dispo-control-text' style='width:80px; text-align:left; font-weight:bold'></td>  
</tr>
</table>
</div>
</td>
...
<div class="cc-mstyle" style="position:fixed;width:500px;height:200px;margin-left:-250px;margin-top:-100px;left:50%;top:50%;background-color:white;z-index:<?php $zi++; echo $zi ?>;" id="TransferMain">
    <table >
    <tr valign="top">
...
<span style="border-radius:0;width:20%;height:35%;position:absolute;left:40%;top:38.5%;z-index:<?php $zi++; echo $zi ?>;" id="TimerSpan" class='cc-mstyle'> 
...
<div class=cc-mstyle style="position:fixed;left:50%;top:50%;z-index:1001;  height:610px; width:1000px; margin-left: -500px; margin-top: -305px;" class="popup_form" id="CallBacKsLisTBox" >
    ...
    <table style="width:95%;height:5%;table-layout:fixed;margin:10px auto;" >
        <tr>
            ...
            <td>
                <h3> Lista de Callbacks </h3>
            </td>
            <td>
                <img style="float:right;cursor:pointer;" onclick="CalLBacKsLisTClose();" src="../images/icons/cross_32.png">
            </td>
        </tr>
		<tr>
			...
			<td>
				<span onClick="CalLBacKsLisTCheck();return false;" style="cursor: pointer;">
...
<table style="margin:10px auto;width:95%">
    <tr>
        <td colspan="3"><img src="/images/icons/telephone_go_32.png" style="float:left">
            <b>Chamada Manual</b>
            <img src="/images/icons/cross_32.png" onClick="ManualDialHide();" style="float:right;cursor:pointer;">
        </td>
    </tr>
    <tr>
        <td colspan="3">
...
<span class="cc-mstyle" style="position: fixed;box-shadow: 0px 0px 120px 10000px rgba(0, 0, 0, 0.5); left: 50%; top: 50%;margin-left:-175px;margin-top:-250px; width: 350px;height:500px; z-index:<?php $zi++; echo $zi ?>;" id="CloserSelectBox">
    <table border="0"> <!-- width="<?php echo $CAwidth ?>px" height="<?php echo $WRheight ?>px" --><tr><td align="center" valign="top"> CLOSER INBOUND GROUP SELECTION <br />
	<?php
	if ($webphone_location == 'bar')
        {echo "<br /><img src=\"images/pixel.gif\" width=\"1px\" height=\"".$webphone_height."px\" /><br />\n";}
	?>
