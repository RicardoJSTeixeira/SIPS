...
function volume_control(taskdirection, taskvolchannel, taskagentmute) {
    ...
    $.post('manager_send.php', livevolume_query);
...
function nc_log() {
    ...
    $.post("/client_files/acusticamedica/novocliente/nc_log.php", {
        inOUT: inOUT_hack,
...
function alert_control(taskalert) {
    ...
    $.post('vdc_db_query.php', alert_query);
...
function stats_update() {
    ...
    if ((VD_live_call_secondS + dead_time + AgentDispoing) < 1) {
        ...
        if (AgentDispoing > 0) {
            ...
            $("#stats_incall").html(VD_live_call_secondS.toString().toHHMMSS());
...
function hangup_recordings(taskconfrec) {
    ...
    $.post('manager_send.php', confhangup_query);
...
function NeWManuaLDiaLCalLSubmiT(tempDiaLnow) {
    ...
    if (AutoDialWaiting) {
        AutoDial_ReSume_PauSe("VDADpause", continueManualDial(tempDiaLnow), '', '', '', '1', auto_pause_precall_code);
        return;
    }
...
function redial() {
    ...
    if (AutoDialReady == 1 && auto_dial_level > 0) {
        AutoDial_ReSume_PauSe("VDADpause", NewRedialSubmiT(redial_number), '', '', '', '1', auto_pause_precall_code);
        return
    }
    if (segue == 1) {
...
function divchecker(curDiv) {
    var rdytogo = 1;
    if ($("#SearcHForMDisplaYBox").css("display") != 'none' && curDiv != "search") {
        rdytogo = 0;
    }
    if ($("#NeWManuaLDiaLBox").css("display") != 'none' && curDiv != "mdial") {
        rdytogo = 0;
    }
    if ($("#CallBacKsLisTBox").css("display") != 'none' && curDiv != "cback") {
        rdytogo = 0;
    }
    if ($("#CloserSelectBox").css("display") != 'none' && curDiv != "closer") {
        rdytogo = 0;
    }
...
function ManualDialOnly(taskaltnum) {
    ...
    manDiaLonly_query = {
        ...
        dial_method: dial_method,
        agent_log_id: agent_log_id,
        security: document.vicidial_form.security_phrase.value,
            ...
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                ...
                if (recheck_VDIC_array[0] == '1') {
                    ...
                    if (reVDIC_data_VDAC[3] == lastcustchannel) {
                        ...
                        document.getElementById("callchannel").value = reVDIC_data_VDAC[3];
    ...
    $.post("vdc_db_query.php",
        ...
        function (data) {
            ...
            if (check_VDIC_array[0] == '1') {
                ...
                document.getElementById("callchannel").value = VDIC_data_VDAC[3];
...
function DispoHanguPAgaiN() {
    ...
    document.getElementById("callchannel").value = AgaiNHanguPChanneL;
    ...
    if (DispoChoice.length < 1) {
        ...
        if (isCB && (scheduled_callbacks > 0)) {
            ...
            DSupdate_query = {
                ...
                call_notes: document.vicidial_form.call_notes_dispo.value,
                cb_to_other_user: cb_to_other_user,
                cb_to_other_username: cb_to_other_username
            };
...
function PauseCodeSelect_submit(newpausecode) {
    ...
    AutoDial_ReSume_PauSe('VDADpause', callback);
...
function NoneInSession() {
    if ($("#NoneInSessionBox").css("display") === "none") {
...
function CloserSelect_submit() {
    ...
    CSCupdate_query = {
        ...
        user: user,
        pass: pass,
        comments: VU_agent_choose_ingroups_DV,
        closer_blended: VICIDiaL_closer_blended,
        campaign: campaign,
        qm_phone: qm_phone,
        dial_method: dial_method,
        closer_choice: CloserSelectChoices + "-"
    };
...
function callinqueuegrab(CQauto_call_id) {
    if (CQauto_call_id > 0) {
        ...
        if ((AutoDialWaiting == 1) || (VD_live_customer_call == 1) || (alt_dial_active == 1) || (MD_channel_look == 1) || (in_lead_preview_state == 1)) {
            if ((auto_pause_precall == 'Y') && ((agent_pause_codes_active == 'Y') || (agent_pause_codes_active == 'FORCE')) && (AutoDialWaiting == 1) && (VD_live_customer_call != 1) && (alt_dial_active != 1) && (MD_channel_look != 1) && (in_lead_preview_state != 1)) {
                AutoDial_ReSume_PauSe("VDADpause", '', '', '', '', '1', 'GRABCL');
            }
            else {
                move_on = 0;
            ...
            $.post("vdc_db_query.php",
                ...
                function (data) {
                    ...
                    $.each(data, function (i, v) {
                        ...
                        $('#callsinqueuelist').append('<tr><td>' + v.phone + '</td><td>' + v.name + '</td><td>' + v.time + '</td><td><span class="btn btn-success btn-mini icon-alone" onclick="callinqueuegrab(\'' + v.call_id + '\')"><i class="icon-phone"></i></span></td></tr>');
        ...
        function (data) {
            {
                ...
                if ((MDnextCID.match(regMNCvar)) || (MDnextCID.match(regMDFvarDNC)) || (MDnextCID.match(regMDFvarCAMP)) || (MDnextCID.match(regMDFvarTIME))) {
                    ...
                    if (starting_dial_level == 0) {
                        $("#DiaLControl").html(DiaLControl_manual_HTML);
    ...
    $.post("../client_files/necomplus/soap_api.php", {phone: apiPhone}, function (data) {
        ...
        if (data.codresultado == '0') {
            ...
            for (i = 0; i < data.datos_ns.contador_ns; i++) {
                var a = i + 1;
                $('#tpaPi tbody').append('<tr><td>' + ((typeof data.datos_ns['id_tpa_' + a] === 'object') ? 'Sem dados' : data.datos_ns['id_tpa_' + a]) + '</td><td>' + ((typeof data.datos_ns['num_serie_' + a] === 'object') ? 'Sem dados' : data.datos_ns['num_serie_' + a]) + '</td></tr>');
            }
            var b = 1;
            for (i = 1; i < 6; i++) {
            ...
            $("#nomePi").html('Sem dados do cliente');
        }
    }, 'json');
...
function VieWLeaDInfO(VLI_lead_id, VLI_cb_id) {
    ...
    $.post("vdc_db_query.php",
        ...
        function (data) {
            ...
            if (cb_info) {
                spot.find("#cb_status").text(cb_info.status).closest("div").show();
                ;
                ...
                spot.find("#cb_status").closest("div").hide();
            }
...
$(document).on("click", ".validar_chamada", function () {
    $("#validar_chamada_modal #validar_chamada_button").data("lead_id", $(this).data("lead_id"));
    $("#validar_chamada_modal #vc_comment").val("");
    $("#validar_chamada_modal").modal("show");
});

$(document).on("click", "#validar_chamada_button", function () {

    $.post("ajax/confirm_feedback.php", {
            action: "validate_call",
            lead_id: $(this).data("lead_id"),
            comment: $("#vc_comment").val(),
            user: user,
            campaign: campaign
        },
        function (data) {
            $("#confirm_feedback_log").hide();
        }, "json");
});
                ...
                if (this.has_info) {
                    $("#comment_log_tbody").append($("<tr>")
                            ...
                            .append($("<td>").text(moment(this.date).fromNow())
                                .append($("<div>").addClass("view-button").append($("<button>").addClass("btn btn-mini validar_chamada icon-ok-sign").data("lead_id", this.lead_id).text(" Validar")))
                                .append($("<div>").addClass("view-button").append($("<button>").addClass("btn btn-mini ligar_comment_log icon-phone").data("lead_id", this.lead_id).data("phone_number", this.phone_number).text(" ligar"))))
                    );
                }
            });
        }, "json");
