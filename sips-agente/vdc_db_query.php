...
if (isset($_GET["extra1"])) {$extra1 = $_GET["extra1"];
} elseif (isset($_POST["extra1"])) {$extra1 = $_POST["extra1"];}

if (isset($_GET["extra2"])) {$extra2 = $_GET["extra2"];
} elseif (isset($_POST["extra2"])) {$extra2 = $_POST["extra2"];}

if (isset($_GET["extra3"])) {$extra3 = $_GET["extra3"];
} elseif (isset($_POST["extra3"])) {$extra3 = $_POST["extra3"];}

if (isset($_GET["extra4"])) {$extra4 = $_GET["extra4"];
} elseif (isset($_POST["extra4"])) {$extra4 = $_POST["extra4"];}

if (isset($_GET["extra5"])) {$extra5 = $_GET["extra5"];
} elseif (isset($_POST["extra5"])) {$extra5 = $_POST["extra5"];}

if (isset($_GET["extra6"])) {$extra6 = $_GET["extra6"];
} elseif (isset($_POST["extra6"])) {$extra6 = $_POST["extra6"];}

if (isset($_GET["extra7"])) {$extra7 = $_GET["extra7"];
} elseif (isset($_POST["extra7"])) {$extra7 = $_POST["extra7"];}

if (isset($_GET["extra8"])) {$extra8 = $_GET["extra8"];
} elseif (isset($_POST["extra8"])) {$extra8 = $_POST["extra8"];}

if (isset($_GET["extra9"])) {$extra9 = $_GET["extra9"];
} elseif (isset($_POST["extra9"])) {$extra9 = $_POST["extra9"];}

if (isset($_GET["extra10"])) {$extra10 = $_GET["extra10"];
} elseif (isset($_POST["extra10"])) {$extra10 = $_POST["extra10"];}

if (isset($_GET["extra11"])) {$extra11 = $_GET["extra11"];
} elseif (isset($_POST["extra11"])) {$extra11 = $_POST["extra11"];}

if (isset($_GET["extra12"])) {$extra12 = $_GET["extra12"];
} elseif (isset($_POST["extra12"])) {$extra12 = $_POST["extra12"];}

if (isset($_GET["extra13"])) {$extra13 = $_GET["extra13"];
} elseif (isset($_POST["extra13"])) {$extra13 = $_POST["extra13"];}

if (isset($_GET["extra14"])) {$extra14 = $_GET["extra14"];
} elseif (isset($_POST["extra14"])) {$extra14 = $_POST["extra14"];}

if (isset($_GET["extra15"])) {$extra15 = $_GET["extra15"];
} elseif (isset($_POST["extra15"])) {$extra15 = $_POST["extra15"];}
...
if (isset($_GET["cb_id"])) {$cb_id = $_GET["cb_id"];
} elseif (isset($_POST["cb_id"])) {$cb_id = $_POST["cb_id"];
}
	
if (isset($_GET["ultimo_callback"]))			{$ultimo_callback=$_GET["ultimo_callback"];}
	elseif (isset($_POST["ultimo_callback"]))	{$ultimo_callback=$_POST["ultimo_callback"];}
	
if (isset($_GET["search_field"]))			{$search_field=$_GET["search_field"];}
	elseif (isset($_POST["search_field"]))	{$search_field=$_POST["search_field"];}
	
if (isset($_GET["search_query"]))			{$search_query=$_GET["search_query"];}
	elseif (isset($_POST["search_query"]))	{$search_query=$_POST["search_query"];}
	
if (isset($_GET["cb_date_1"]))			{$cb_date_1=$_GET["cb_date_1"];}
	elseif (isset($_POST["cb_date_1"]))	{$cb_date_1=$_POST["cb_date_1"];}
if (isset($_GET["cb_date_2"]))			{$cb_date_2=$_GET["cb_date_2"];}
	elseif (isset($_POST["cb_date_2"]))	{$cb_date_2=$_POST["cb_date_2"];}
	
if (isset($_GET["portabilidade"]))	{$portabilidade=$_GET["portabilidade"];}
	elseif (isset($_POST["portabilidade"]))	{$portabilidade=$_POST["portabilidade"];}
	
if (isset($_GET["cp_4"]))	{$cp_4=$_GET["cp_4"];}
...
if ($ACTION == 'LogiNCamPaigns') {
	...
	if ((strlen($pass) < 1)) {
		...
		if ((ereg('Y', $forced_timeclock_login)) or ((ereg('ADMIN_EXEMPT', $forced_timeclock_login)) and ($VU_user_level < 8))) {
			...
			if ($HHMM < $timeclock_end_of_day) {$EoD = mktime($HHteod, $MMteod, 10, date("m"), date("d") - 1, date("Y"));
			} else {$EoD = mktime($HHteod, $MMteod, 10, date("m"), date("d"), date("Y"));
			}
			...
			if ((strlen($last_agent_event) < 2) or (ereg('LOGOUT', $last_agent_event))) {$show_campaign_list = 0;
			}
		}
	}
		...
		if ((strlen($LOGgroup_shiftsSQL) < 3) and ($VU_shift_override_flag < 1)) {
			...
			while (($shifts_to_print > $o) and ($shift_ok < 1)) {
				...
				if (eregi("$wday", $shift_weekdays)) {
					...
					if ($HHshift_end_time > 23) {$HHshift_end_time = ($HHshift_end_time - 24);
					}
					...
					if ((($HHMM >= $shift_start_time) and ($HHMM < $shift_end_time)) or (($HHMM < $shift_start_time) and ($HHMM < $shift_end_time) and ($shift_end_time <= $shift_start_time)) or (($HHMM >= $shift_start_time) and ($HHMM >= $shift_end_time) and ($shift_end_time <= $shift_start_time))) {$shift_ok++;
					}
				}
				$o++;
			}
	...
	if ($show_campaign_list > 0) {
		...
		if ($valogin == 0) {
			...
			echo "<select style=width:200px name=VD_campaign id=VD_campaign>\n";
...
if ($ACTION == 'regCLOSER') {
	...
	if ((strlen($closer_choice) < 1) || (strlen($user) < 1)) {
		...
		if ($closer_blended > 0) {$vla_autodial = 'Y';
		} else {$vla_autodial = 'N';
		}
		if (preg_match('/INBOUND_MAN|MANUAL/', $dial_method)) {$vla_autodial = 'N';
		}
	...
	if ((strlen($conf_exten) < 1) || (strlen($campaign) < 1) || (strlen($ext_context) < 1)) {
		...
		elseif (strlen($lead_id) > 0) {
			...
			if (strlen($phone_number) > 3) {
				...
				if (ereg("DNC", $manual_dial_filter)) {
					...
					} else {$stmt = "SELECT count(*) FROM vicidial_dnc where phone_number='$phone_number';";
					}
					$rslt = mysql_query($stmt, $link);
					...
					} else {$stmt = "SELECT count(*) FROM vicidial_campaign_dnc where phone_number='$phone_number' and campaign_id='$campaign';";
					}
					$rslt = mysql_query($stmt, $link);
				...
				if ($stage == 'lookup') {
					if (strlen($vendor_lead_code) > 0) {
						...
						if (($man_leadID_ct > 0) and (strlen($phone_number) > 5)) {$override_phone++;
						}
					} else {
					...
					if (strlen($dial_statuses) > 2) {
								...
								while ($r < $g) {
									if ($GMT_day[$r] == 0)#### Sunday local time
									{
										if (($Gsct_sunday_start == 0) and ($Gsct_sunday_stop == 0)) {
											if (($GMT_hour[$r] >= $Gsct_default_start) and ($GMT_hour[$r] < $Gsct_default_stop)) {$state_gmt .= "'$GMT_gmt[$r]',";
											}
										} else {
											if (($GMT_hour[$r] >= $Gsct_sunday_start) and ($GMT_hour[$r] < $Gsct_sunday_stop)) {$state_gmt .= "'$GMT_gmt[$r]',";
											}
										}
									}
									if ($GMT_day[$r] == 1)#### Monday local time
									{
										if (($Gsct_monday_start == 0) and ($Gsct_monday_stop == 0)) {
											...
											if (($GMT_hour[$r] >= $Gsct_monday_start) and ($GMT_hour[$r] < $Gsct_monday_stop)) {$state_gmt .= "'$GMT_gmt[$r]',";
											}
										}
									}
									if ($GMT_day[$r] == 2)#### Tuesday local time
									{
										if (($Gsct_tuesday_start == 0) and ($Gsct_tuesday_stop == 0)) {
											...
											if (($GMT_hour[$r] >= $Gsct_tuesday_start) and ($GMT_hour[$r] < $Gsct_tuesday_stop)) {$state_gmt .= "'$GMT_gmt[$r]',";
											}
										}
									}
									if ($GMT_day[$r] == 3)#### Wednesday local time
									{
										if (($Gsct_wednesday_start == 0) and ($Gsct_wednesday_stop == 0)) {
											...
											if (($GMT_hour[$r] >= $Gsct_wednesday_start) and ($GMT_hour[$r] < $Gsct_wednesday_stop)) {$state_gmt .= "'$GMT_gmt[$r]',";
											}
										}
									}
									if ($GMT_day[$r] == 4)#### Thursday local time
									{
										if (($Gsct_thursday_start == 0) and ($Gsct_thursday_stop == 0)) {
											...
											if (($GMT_hour[$r] >= $Gsct_thursday_start) and ($GMT_hour[$r] < $Gsct_thursday_stop)) {$state_gmt .= "'$GMT_gmt[$r]',";
											}
										}
									}
									if ($GMT_day[$r] == 5)#### Friday local time
									{
										if (($Gsct_friday_start == 0) and ($Gsct_friday_stop == 0)) {
											...
											if (($GMT_hour[$r] >= $Gsct_friday_start) and ($GMT_hour[$r] < $Gsct_friday_stop)) {$state_gmt .= "'$GMT_gmt[$r]',";
											}
										}
									}
									if ($GMT_day[$r] == 6)#### Saturday local time
									{
										if (($Gsct_saturday_start == 0) and ($Gsct_saturday_stop == 0)) {
											...
											if (($GMT_hour[$r] >= $Gsct_saturday_start) and ($GMT_hour[$r] < $Gsct_saturday_stop)) {$state_gmt .= "'$GMT_gmt[$r]',";
											}
										}
									}
									$r++;
						...
						while ($r < $g) {
							if ($GMT_day[$r] == 0)#### Sunday local time
							{
								if (($Gct_sunday_start == 0) and ($Gct_sunday_stop == 0)) {
									if (($GMT_hour[$r] >= $Gct_default_start) and ($GMT_hour[$r] < $Gct_default_stop)) {$default_gmt .= "'$GMT_gmt[$r]',";
									}
								} else {
									if (($GMT_hour[$r] >= $Gct_sunday_start) and ($GMT_hour[$r] < $Gct_sunday_stop)) {$default_gmt .= "'$GMT_gmt[$r]',";
									}
								}
							}
							if ($GMT_day[$r] == 1)#### Monday local time
							{
								if (($Gct_monday_start == 0) and ($Gct_monday_stop == 0)) {
									...
									if (($GMT_hour[$r] >= $Gct_monday_start) and ($GMT_hour[$r] < $Gct_monday_stop)) {$default_gmt .= "'$GMT_gmt[$r]',";
									}
								}
							}
							if ($GMT_day[$r] == 2)#### Tuesday local time
							{
								if (($Gct_tuesday_start == 0) and ($Gct_tuesday_stop == 0)) {
									...
									if (($GMT_hour[$r] >= $Gct_tuesday_start) and ($GMT_hour[$r] < $Gct_tuesday_stop)) {$default_gmt .= "'$GMT_gmt[$r]',";
									}
								}
							}
							if ($GMT_day[$r] == 3)#### Wednesday local time
							{
								if (($Gct_wednesday_start == 0) and ($Gct_wednesday_stop == 0)) {
									...
									if (($GMT_hour[$r] >= $Gct_wednesday_start) and ($GMT_hour[$r] < $Gct_wednesday_stop)) {$default_gmt .= "'$GMT_gmt[$r]',";
									}
								}
							}
							if ($GMT_day[$r] == 4)#### Thursday local time
							{
								if (($Gct_thursday_start == 0) and ($Gct_thursday_stop == 0)) {
									...
									if (($GMT_hour[$r] >= $Gct_thursday_start) and ($GMT_hour[$r] < $Gct_thursday_stop)) {$default_gmt .= "'$GMT_gmt[$r]',";
									}
								}
							}
							if ($GMT_day[$r] == 5)#### Friday local time
							{
								if (($Gct_friday_start == 0) and ($Gct_friday_stop == 0)) {
									...
									if (($GMT_hour[$r] >= $Gct_friday_start) and ($GMT_hour[$r] < $Gct_friday_stop)) {$default_gmt .= "'$GMT_gmt[$r]',";
									}
								}
							}
							if ($GMT_day[$r] == 6)#### Saturday local time
							{
								if (($Gct_saturday_start == 0) and ($Gct_saturday_stop == 0)) {
									...
									if (($GMT_hour[$r] >= $Gct_saturday_start) and ($GMT_hour[$r] < $Gct_saturday_stop)) {$default_gmt .= "'$GMT_gmt[$r]',";
									}
								}
							}
							$r++;
						...
						if (eregi("TERRITORY", $agent_dial_owner_only)) {
							...
							if ($agent_choose_territories < 1) {
								...
								if ($vuts_to_parse < 1) {$searchownerSQL = " and lead_id < 0";
								}
							} else {
		...
		if ($affected_rows > 0) {
			...
			if ($list_lead_ct > 0) {
				...
				if ($override_phone < 1) {$phone_number = trim("$row[11]");
				}
				$title = trim("$row[12]");
			...
			if ((strlen($agent_dialed_type) < 3) or (strlen($agent_dialed_number) < 6)) {
				...
				if (strlen($agent_dialed_type) < 3) {$agent_dialed_type = 'MAIN';
				}
			}
			if ((strlen($callback_id) > 0) and (strlen($lead_id) > 0)) {
				if ($agent_dialed_type == 'ALT') {$agent_dialed_number = $alt_phone;
				}
				if ($agent_dialed_type == 'ADDR3') {$agent_dialed_number = $address3;
				}
			}
			...
			if (($post_phone_time_diff_alert == 'ENABLED') or (preg_match("/OUTSIDE_CALLTIME/", $post_phone_time_diff_alert))) {
				...
				if ($PHONEgmt_offset != $POSTgmt_offset) {
					...
					if ($post_phone_time_diff_alert == 'OUTSIDE_CALLTIME_ONLY') {
						$post_phone_time_diff_alert_message = '';
						if ($PHONEdialable < 1) {$post_phone_time_diff_alert_message .= " Phone Area Code Outside Dialable Zone $PHONEgmt_offset ";
						}
						if ($POSTdialable < 1) {$post_phone_time_diff_alert_message .= " Postal Code Outside Dialable Zone $POSTgmt_offset";
						}
					}
				}
				if (($post_phone_time_diff_alert == 'OUTSIDE_CALLTIME_PHONE') or ($post_phone_time_diff_alert == 'OUTSIDE_CALLTIME_POSTAL') or ($post_phone_time_diff_alert == 'OUTSIDE_CALLTIME_BOTH')) {$post_phone_time_diff_alert_message = '';
				}
			...
			if ((strlen($preview) < 1) || ($preview == 'NO')) {
				...
				if (strlen($campaign_cid) > 6) {$CCID = "$campaign_cid";
					$CCID_on++;
				}
				if (strlen($campaign_cid_override) > 6) {$CCID = "$campaign_cid_override";
					$CCID_on++;
				}
				...
				if ($use_custom_cid > 0) {
					...
					if (strlen($temp_CID) > 6) {$CCID = "$temp_CID";
						$CCID_on++;
					}
				}
				...
				if($portabilidade>0)
				{
					switch($portabilidade) {
						case "1": $agent_dialed_number = "96".$agent_dialed_number; break;
						case "2":  $agent_dialed_number =  "91".$agent_dialed_number; break;
						case "3":  $agent_dialed_number =  "93".$agent_dialed_number; break;
					}	
				} 
				
				
				
				if (eregi('Y', $omit_phone_code)) {$Ndialstring = "$Local_out_prefix$agent_dialed_number";
				} else {$Ndialstring = "$Local_out_prefix$phone_code$agent_dialed_number";
				}
			...
			if (strlen($list_id) > 0) {
				...
				if ($VDIG_preset_ct > 0) {
					$row = mysql_fetch_row($rslt);
					if (strlen($row[0]) > 0) {$VDCL_xferconf_a_number = $row[0];
					}
					if (strlen($row[1]) > 0) {$VDCL_xferconf_b_number = $row[1];
					}
					if (strlen($row[2]) > 0) {$VDCL_xferconf_c_number = $row[2];
					}
					if (strlen($row[3]) > 0) {$VDCL_xferconf_d_number = $row[3];
					}
					if (strlen($row[4]) > 0) {$VDCL_xferconf_e_number = $row[4];
					}
				}
...
if ($ACTION == 'manDiaLonly') {
	...
	if ((strlen($conf_exten) < 1) || (strlen($campaign) < 1) || (strlen($ext_context) < 1) || (strlen($phone_number) < 1) || (strlen($lead_id) < 1)) {
		...
		### whether to omit phone_code or not
				...
				{
					switch($portabilidade) {
						case "1": $phone_number = "96".$phone_number; break;
						case "2":  $phone_number =  "91".$phone_number; break;
						case "3":  $phone_number =  "93".$phone_number; break;
					}	
				} 
                
		if (eregi('Y', $omit_phone_code)) {$Ndialstring = "$Local_out_prefix$phone_number";
	...
	if (strlen($MDnextCID) < 18) {
		...
		if ($call_good > 0) {
			...
			if ($VDpr_ct > 0) {
				...
				if (($now_dead_epoch > 1000) and ($now_dead_epoch < $StarTtime)) {$dead_epochSQL = ",dead_epoch='$StarTtime'";
				}
			}
			...
			if ($auto_dial_level > 0) {
				...
				if (eregi("(ALT_ONLY|ADDR3_ONLY|ALT_AND_ADDR3|ALT_AND_EXTENDED|ALT_AND_ADDR3_AND_EXTENDED|EXTENDED_ONLY)", $auto_alt_dial)) {
					...
					if ((eregi("(NONE|MAIN)", $alt_dial)) and (eregi("(ALT_ONLY|ALT_AND_ADDR3|ALT_AND_EXTENDED)", $auto_alt_dial))) {
						...
						if (strlen($alt_phone) > 5) {
							if ((ereg("Y", $use_internal_dnc)) or (ereg("AREACODE", $use_internal_dnc))) {
								...
								} else {$stmtA = "SELECT count(*) FROM vicidial_dnc where phone_number='$alt_phone';";
								}
							...
							if ((ereg("Y", $use_campaign_dnc)) or (ereg("AREACODE", $use_campaign_dnc))) {
								...
								} else {$stmtA = "SELECT count(*) FROM vicidial_campaign_dnc where phone_number='$alt_phone' and campaign_id='$campaign';";
								}
								$rslt = mysql_query($stmtA, $link);
					...
					if (((eregi("(ALT)", $alt_dial)) and (eregi("ALT_AND_ADDR3", $auto_alt_dial))) or ((eregi("(NONE|MAIN)", $alt_dial)) and (eregi("ADDR3_ONLY", $auto_alt_dial)))) {
						...
						if (strlen($address3) > 5) {
							if ((ereg("Y", $use_internal_dnc)) or (ereg("AREACODE", $use_internal_dnc))) {
								...
								} else {$stmtA = "SELECT count(*) FROM vicidial_dnc where phone_number='$address3';";
								}
								$rslt = mysql_query($stmtA, $link);
							...
							if ((ereg("Y", $use_campaign_dnc)) or (ereg("AREACODE", $use_campaign_dnc))) {
								...
								} else {$stmtA = "SELECT count(*) FROM vicidial_campaign_dnc where phone_number='$address3' and campaign_id='$campaign';";
								}
								$rslt = mysql_query($stmtA, $link);
					...
					if ((((eregi("(NONE|MAIN)", $alt_dial)) and (eregi("EXTENDED_ONLY", $auto_alt_dial))) or ((eregi("(ALT)", $alt_dial)) and (eregi("(ALT_AND_EXTENDED)", $auto_alt_dial))) or ((eregi("(ADDR3)", $alt_dial)) and (eregi("(ADDR3_AND_EXTENDED|ALT_AND_ADDR3_AND_EXTENDED)", $auto_alt_dial))) or ((eregi("(X)", $alt_dial)) and (eregi("EXTENDED", $auto_alt_dial)))) and (!eregi("LAST", $alt_dial))) {
						...
						} else {$Xlast = ereg_replace("[^0-9]", "", $alt_dial);
						}
						if (strlen($Xlast) < 1) {$Xlast = 0;
						}
						$VD_altdialx = '';
						...
						while (($alt_dial_phones_count > 0) and ($alt_dial_phones_count > $Xlast)) {
							...
							if (ereg("Y", $VD_altdial_active)) {
								if ((ereg("Y", $use_internal_dnc)) or (ereg("AREACODE", $use_internal_dnc))) {
									...
									} else {$stmtA = "SELECT count(*) FROM vicidial_dnc where phone_number='$VD_altdial_phone';";
									}
									$rslt = mysql_query($stmtA, $link);
								...
								if ((ereg("Y", $use_campaign_dnc)) or (ereg("AREACODE", $use_campaign_dnc))) {
									...
									} else {$stmtA = "SELECT count(*) FROM vicidial_campaign_dnc where phone_number='$VD_altdial_phone' and campaign_id='$campaign';";
									}
									$rslt = mysql_query($stmtA, $link);
								...
								if ($VD_alt_dnc_count < 1) {
									if ($alt_dial_phones_count == $Xlast) {$Xlast = 'LAST';
									}
				...
				if ($enable_queuemetrics_logging > 0) {
					if ((strlen($QL_term) > 0) and ($leaving_threeway > 0)) {
						...
						if ($agent_complete < 1) {
							...
							if (strlen($VDqueue_position) < 1) {$VDqueue_position = 1;
							}
				...
				if (strlen($rec_channels[$loop_count]) > 5) {
					...
					if (strlen($filename[$loop_count]) > 2) {
						...
						if ($fn_count) {
							...
							if ((strlen($RClead_id) < 1) or ($RClead_id < 1) or ($RClead_id == 'NULL')) {$lidSQL = ",lead_id='$lead_id'";
							}
							if (strlen($vicidial_id) < 1) {$vidSQL = ",vicidial_id='$VDvicidial_id'";
							} else {
								if ((ereg('.', $vicidial_id)) and ($VLA_inOUT == 'INBOUND')) {
									if (!ereg('.', $VDvicidial_id)) {$vidSQL = ",vicidial_id='$VDvicidial_id'";
									}
		...
		if ($queue_leadID_ct > 0) {
						...
						if ($dialed_number != $address3) {
							...
							if ($VDAP_cid_ct > 0) {
								...
								if ($VDAPct_cid_ct > 0) {
									...
									if ($COUNTalt_phone_count <= $Xalt_phone_count) {$dialed_label = 'XLAST';
									} else {$dialed_label = "X$Xalt_phone_count";
									}
								}

							}
						} else {$dialed_label = 'ADDR3';
						}
					} else {$dialed_label = 'ALT';
					}
				}
			} else {
			...
			if (($call_type == 'OUT') or ($call_type == 'OUTBALANCE')) {
				...
				if ($VDIG_cid_ct > 0) {
					...
					if ($VDIG_cidOR_ct > 0) {
						...
						if (strlen($VDCL_default_group_alias) < 1) {$VDCL_default_group_alias = $row[5];
						}
						if (strlen($VDCL_timer_action) < 1) {$VDCL_timer_action = $row[6];
						}
						if (strlen($VDCL_timer_action_message) < 1) {$VDCL_timer_action_message = $row[7];
						}
						if (strlen($VDCL_timer_action_seconds) < 1) {$VDCL_timer_action_seconds = $row[8];
						}
						if (strlen($VDCL_start_call_url) < 1) {$VDCL_start_call_url = $row[9];
						}
						if (strlen($VDCL_dispo_call_url) < 1) {$VDCL_dispo_call_url = $row[10];
						}
						if (strlen($VDCL_xferconf_c_number) < 1) {$VDCL_xferconf_c_number = $row[11];
						}
						if (strlen($VDCL_xferconf_d_number) < 1) {$VDCL_xferconf_d_number = $row[12];
						}
						if (strlen($VDCL_xferconf_e_number) < 1) {$VDCL_xferconf_e_number = $row[13];
						}
						if (strlen($VDCL_timer_action_destination) < 1) {$VDCL_timer_action_destination = $row[14];
						}
				...
				if (strlen($list_id) > 0) {
					...
					if ($VDIG_cidOR_ct > 0) {
						...
						if (strlen($row[5]) > 5) {$VDCL_group_web = $row[5];
						}
						if (strlen($row[6]) > 5) {$VDCL_group_web_two = $row[6];
						}
					}
				}
			...
			if (strlen($VDCL_start_call_url) > 7) {
				...
				if ((($enable_vtiger_integration > 0) and (ereg('callxfer', $VDCL_start_call_url)) and (ereg('contactwsid', $VDCL_start_call_url))) or (preg_match("/minuteswarning/", $VDCL_start_call_url))) {
					$SCUoutput = '';
					foreach ($SCUfile as $SCUline) {$SCUoutput .= "$SCUline";
					}
	...
	if ((strlen($campaign) < 1) || (strlen($conf_exten) < 1)) {
		...
		if ($vla_delete > 0) {
			...
			if ($enable_queuemetrics_logging > 0) {
				...
				if ($queuemetrics_loginout == 'CALLBACK') {$QM_LOGOFF = 'AGENTCALLBACKLOGOFF';
				}
...
if ($ACTION == 'updateDISPO') {
				...
				if ($row[0] > 0) {
					...
					if ($VLinfo_ct > 0) {
						...
						if (strlen($phone_number) < 6) {
							...
							if ($phone_number == "$row[1]") {$VLalt = 'MAIN';
							} else {
									...
									if ($phone_number != $VLaddress3) {
										...
										if ($VDAP_cid_ct > 0) {
											...
											if ($VDAPct_cid_ct > 0) {
												...
												if ($COUNTalt_phone_count <= $Xalt_phone_count) {$VLalt = 'XLAST';
												} else {$VLalt = "X$Xalt_phone_count";
												}
											}
						...
						if (strlen($phone_code) < 1) {$VLphone_code = "$row[2]";
						} else {$VLphone_code = "$phone_code";
						}
					}
	...
	if ($auto_dial_level < 1) {
		...
		if ($MAN_vl_insert > 0) {$MAN_insert_leadIDsql = ",lead_id='$lead_id'";
		}
	...
	if (($auto_dial_level > 0) and (ereg(" $dispo_choice ", $VC_auto_alt_dial_statuses))) {
		...
		if ($row[0] > 0) {
			...
			if ((ereg("Y", $use_internal_dnc)) or (ereg("AREACODE", $use_internal_dnc))) {
				...
				} else {$stmtA = "SELECT count(*) FROM vicidial_dnc where phone_number='$vh_phone';";
				}
				$rslt = mysql_query($stmtA, $link);
			...
			if ((ereg("Y", $use_campaign_dnc)) or (ereg("AREACODE", $use_campaign_dnc))) {
				...
				} else {$stmtA = "SELECT count(*) FROM vicidial_campaign_dnc where phone_number='$vh_phone' and campaign_id='$campaign';";
				}
				$rslt = mysql_query($stmtA, $link);
...
if( mysql_num_rows( mysql_query("SHOW TABLES LIKE 'sips_status_action'")))
{
		...
		if ((ereg('ACCTID', $vtiger_search_category)) or (ereg('ACCOUNT', $vtiger_search_category))) {
			...
			if ($VIDcount > 0) {
				...
				if (ereg('Y', $vtiger_status_call)) {
					...
					$VT_last_sale_ct = 0;
					if (eregi("\|$dispo_choice\|", $SALE_string_check)) {$VT_last_sale_update++;
					}
					if (eregi("\|$dispo_choice\|", $CC_string_check)) {$VT_last_contact_update++;
					} else {$VT_last_noncontact_update++;
					}
	...
	if (($enable_queuemetrics_logging > 0) and ((($queuemetrics_callstatus > 0) or ($queuemetrics_callstatus_override == 'YES')) and ($queuemetrics_callstatus_override != 'NO'))) {
		...
		if (strlen($stage) < 2) {$stage = $campaign;
		}
...
if ($ACTION == 'updateLEAD') {
	...
	if ((strlen($phone_number) < 1) || (strlen($lead_id) < 1)) {
		...
		if ($DO_NOT_UPDATE < 1) {
			...
			$phoneSQL = '';
			if ($DO_NOT_UPDATEphone < 1) {$phoneSQL = ",phone_number='$phone_number'";
			}
	...
	if($qResult[0]==null || $qResult[0]=='') { $query="UPDATE vicidial_list SET last_local_call_time=entry_date WHERE lead_id='$lead_id'"; $query=mysql_query($query); }
}
...
if (($ACTION == 'VDADpause') || ($ACTION == 'VDADready')) {
	...
	if ((strlen($stage) < 2) || (strlen($server_ip) < 1)) {
		...
		$vla_autodialSQL = '';
		if (preg_match('/INBOUND_MAN/', $dial_method)) {$vla_autodialSQL = ",outbound_autodial='N'";
		}
		$random = (rand(1000000, 9999999) + 10000000);
		...
		if ($comments != 'NO_STATUS_CHANGE') {
			$vla_lead_wipeSQL = '';
			if ($ACTION == 'VDADready') {$vla_lead_wipeSQL = ",lead_id=0";
			}
			if ($ACTION == 'VDADpause') {$vla_ring_resetSQL = ",ring_callerid=''";
			}
		...
		if ($VDpr_ct > 0) {
			...
			if ((eregi("NULL", $row[4])) or ($row[4] < 1000)) {$pause_sec = (($StarTtime - $row[0]) + $row[1]);
			} else {$pause_sec = (($row[4] - $row[0]) + $row[1]);
			}
...
if ($ACTION == 'AGENTSview') {
	...
	$agent_status_view = 0;
	if (strlen($agent_status_viewable_groups) > 2) {$agent_status_view = 1;
	}
	$agent_status_view_time = 0;
	if ($row[1] == 'Y') {$agent_status_view_time = 1;
	}
	$andSQL = '';
	if (ereg("ALL-GROUPS", $agent_status_viewable_groups)) {$AGENTviewSQL = "";
	} else {
		$AGENTviewSQL = "($agent_status_viewable_groupsSQL)";

		if (ereg("CAMPAIGN-AGENTS", $agent_status_viewable_groups)) {$AGENTviewSQL = "($AGENTviewSQL or (campaign_id='$campaign'))";
		}
		$AGENTviewSQL = "and $AGENTviewSQL";
	}
	if ($comments == 'AgentXferViewSelect') {$AGENTviewSQL .= " and (vla.closer_campaigns LIKE \"%AGENTDIRECT%\")";
	}
	...
	while ($agents_count > $loop_count) {
		...
		if ($comments == 'AgentXferViewSelect') {
			...
			echo "$row[0] - $row[2]";
			echo "&nbsp;</font></TD>";
			if ($agent_status_view_time > 0) {echo "<TD><font style=\"font-size: 12px;  font-family: sans-serif;\">&nbsp; $call_time &nbsp;</font></TD>";
			}
			echo "</TR>";
		}
		$loop_count++;
	...
	if (strlen($agentviewlistSQL) < 3) {$agentviewlistSQL = "''";
	}
	...
	if (ereg("NOT-LOGGED-IN-AGENTS", $agent_status_viewable_groups)) {
		...
		while ($agents_count > $loop_count) {
			...
			if ($comments == 'AgentXferViewSelect') {
				...
				echo "$user - $full_name";
				echo "&nbsp;</font></TD>";
				if ($agent_status_view_time > 0) {echo "<TD><font style=\"font-size: 12px;  font-family: sans-serif;\">&nbsp; 0:00 &nbsp;</font></TD>";
				}
				echo "</TR>";
			}
			$loop_count++;
	...
	if ($comments == 'AgentXferViewSelect') {
		...
		while ($j > $k) {
			...
			if ($agent_status_view_time > 0) {echo "<TD><font style=\"font-size: $AXVSfontsize;  font-family: sans-serif;\">&nbsp; $AXVScall_time[$i] &nbsp;</font></TD>";
			}
			echo "</TR>";
...
if ($ACTION == 'CALLSINQUEUEview') {
	...
	if (eregi('NONE', $view_calls_in_queue)) {
		...
		while (($calls_count > $loop_count) and ($view_calls_in_queue > $loop_count)) {
			...
			if (strlen($caller_name) < 2) {$caller_name = $comments;
			}
...
if ($ACTION == 'CALLLOGview') {
	if (strlen($date) < 10) {$date = $NOW_DATE;
	}
	if (strlen($stage) < 3) {$stage = '670';
	}
	...
	if ($NOW_DATE != $date) {echo "<a href=\"#\" onclick=\"VieWCalLLoG('$next_day_date','');return false;\"> $next_day_date > </a> &nbsp; &nbsp; ";
	}
	...
	if ($g > 0) {sort($ALLsort, SORT_NUMERIC);
	} else {echo "<tr bgcolor=white><td colspan=11 align=center>No calls on this day</td></tr>";
	}
	...
	while ($g > $u) {
		...
		if (eregi("1$|3$|5$|7$|9$", $u)) {$bgcolor = 'bgcolor="#B9CBFD"';
		} else {$bgcolor = 'bgcolor="#9BB9FB"';
		}
...
if ($ACTION == 'SEARCHRESULTSview') {
	...
	$postal_code = ereg_replace("'|\"|\\\\|;", "", $postal_code);
		...
		if (strlen($search_query)<1) {
				...
				echo "ERRO: O campo de pesquisa não pode estar vazio.\n";
				echo "<BR><BR>";
	...
	/*if (strlen($lead_id) > 0) {
		...
		$searchSQL = "lead_id LIKE '%$lead_id%'";
	} elseif (strlen($vendor_lead_code) > 0) {
		### vendor ID entered, search by this
		$searchSQL = "vendor_lead_code LIKE '%$vendor_lead_code%'";
	} elseif ((strlen($phone_number) >= 6) and (strlen($search) > 2)) {
		### phone number entered, search by this
		if (preg_match('/MAIN/', $search)) {$searchSQL = "phone_number LIKE '%$phone_number%'";
		}
		if (preg_match('/ALT/', $search)) {
			if (strlen($searchSQL) > 10) {$searchSQL .= " or ";
			}
		...
		if (preg_match('/ADDR3/', $search)) {
			...
			$searchSQL .= "address3 LIKE '%$phone_number%'";
		}
		...
		if (strlen($city) > 0) {
			...
			$searchSQL .= "city LIKE '%$city%'";
		}
		if (strlen($state) > 0) {
			if (strlen($searchSQL) > 10) {$searchSQL .= " and ";
			}
			$searchSQL .= "state LIKE '%$state%'";
		}
		if (strlen($postal_code) > 0) {
			if (strlen($searchSQL) > 10) {$searchSQL .= " and ";
			}
			$searchSQL .= "postal_code LIKE '%$postal_code%'";
		}
	} else {
		echo "ERRO: Deve preencher pelo menos, o número de telefone, ou o ultimo nome.\n";
		echo "<BR><BR>";
		echo "<a href=\"#\" onclick=\"hideDiv('SearcHResultSDisplaYBox');return false;\">Go Back</a>";
		echo "</CENTER>";
		exit ;
	...
	if ($counts_to_print > 0) {
		...
		echo "<CENTER>\n";
		echo "<B>";
		echo "Resultados encontrados: $search_result_count <br>";
		echo "</B>";
		echo "<BR>\n";
		echo "<DIV class=cc-mstyle style='height: 300px;overflow: scroll;'>";
		...
		if ($search_result_count) {
			...
			$stmt = "select first_name,last_name,phone_code,phone_number,status,last_local_call_time,lead_id,city,state,postal_code from vicidial_list a inner join vicidial_lists b on a.list_id=b.list_id  where ".mysql_real_escape_string($search_field)." like '%".mysql_real_escape_string($search_query)."%' $allowed_camp order by last_local_call_time desc limit 1000;";
			...
			if ($g < 1) {echo "<tr bgcolor=white><td colspan=10 align=center>No results found</td></tr>";
			}
			...
			while ($g > $u) {
				...
				echo "<tr $bgcolor>";
				echo "<td><font size=1>$u</td>";
				echo "<td align=right> $ALLname[$i] </td>\n";
				echo "<td align=right> $ALLphone_code[$i] $ALLphone_number[$i] </td>\n";
				echo "<td align=right> $ALLstatus[$i] </td>\n";
				echo "<td align=right> $ALLcall_date[$i] </td>\n";
				echo "<td align=right> $ALLcity[$i] </td>\n";
				echo "<td align=right> $ALLstate[$i]</td>\n";
				echo "<td align=right> $ALLpostal_code[$i] </td>\n";
				#echo "<td align=right> <a href=\"#\" onclick=\"VieWLeaDInfO($ALLlead_id[$i]);return false;\"><img src=../images/icons/info_rhombus.png /></A> </td>\n";
				echo "<td align=right> <a href=\"#\" onclick=\"NeWManuaLDiaLCalL('LEADSEARCH','$ALLphone_code[$i]','$ALLphone_number[$i]','$ALLlead_id[$i]');return false;\"><img src=../images/icons/phone_call_32.png /></A> </td>\n";
				echo "</tr>\n";
			}
		...
		} else {echo "<tr bgcolor=white><td colspan=10 align=center>Não foram encontrados resultados.</td></tr>";
		}

		echo "<tbody>";
		echo "</TABLE></div>";
		echo "<BR>";
		echo "<table><tr><td style=text-align:right;><a href=\"#\" onclick=\"hideDiv('SearcHResultSDisplaYBox');return false;\"><img src=../images/icons/resultset_previous_32.png></a></td><td style=text-align:left;><a href=\"#\" onclick=\"hideDiv('SearcHResultSDisplaYBox');return false;\">Voltar</a></td></tr></table>";
		echo "</CENTER>";
	} else {
		echo "ERROR: Ouve um erro com a sua pesquisa.\n";
		echo "<BR><BR>";
		echo "<table><tr><td style=text-align:right;><a href=\"#\" onclick=\"hideDiv('SearcHResultSDisplaYBox');return false;\"><img src=../images/icons/resultset_previous_32.png></a></td><td style=text-align:left;><a href=\"#\" onclick=\"hideDiv('SearcHResultSDisplaYBox');return false;\">Voltar</a></td></tr></table>";
		echo "</CENTER>";
		exit ;
...
if ($ACTION == 'LEADINFOview') {
	if (strlen($lead_id) < 1) {echo "ERROR: no Lead ID";
	} else {
		$hide_dial_links = 0;
		echo "<h4>Informação do Callback</h4><Div class='datagrid'>\n";

		if ($search == 'logfirst') {$hide_dial_links++;
		}
		...
		if (strlen($callback_id) > 0) {
			...
			if ($cb_to_print > 0) {
				$row = mysql_fetch_row($rslt);
				echo "<TABLE>";
				echo "<tr><th ALIGN=right>Estado de Callback:  </th><td ALIGN=left>$row[0]</td></tr>";
				echo "<tr><th ALIGN=right>Estado de Lead: </th><td ALIGN=left>$row[7]</td></tr>";
				echo "<tr><th ALIGN=right>Entrada de Callback:  </th><td ALIGN=left>$row[1]</td></tr>";
				echo "<tr><th ALIGN=right>Hora de Callback: </th><td ALIGN=left>$row[2]</td></tr>";
				echo "<tr><th ALIGN=right>Comentário de callback: </th><td ALIGN=left>$row[6]</td></tr>";
				echo "</TABLE>";
				echo "</Div><br>";
				$hide_dial_links++;
			}
		...
		$INFOout .= "<h4>Informação da Lead</h4><Div class='datagrid'><TABLE>";
        ...
        for ($i = 0; $i < count($fields_order); $i++) {
            $INFOout .=($fields_order[$i][1]) ? "<tr><th ALIGN=right>" . $fields_order[$i][3] . "</th><td ALIGN=left>" . $row[$fields_order[$i][0]] . "</td></tr>" : "";
        }
		/*if ($info_to_print > 0) {
			$row = mysql_fetch_row($rslt);
			...
			if (($post_phone_time_diff_alert == 'ENABLED') or (preg_match("/OUTSIDE_CALLTIME/", $post_phone_time_diff_alert))) {
				...
				if (strlen($post_phone_time_diff_alert_message) > 5) {$INFOout .= "<tr bgcolor=white><td colspan=2 align=center><font size=2 color=red><b>$post_phone_time_diff_alert_message</b></font></td></tr>";
				}
			}
			##### END check for postal_code and phone time zones if alert enabled

			$INFOout .= "<tr><th ALIGN=right>Status: &nbsp; </th><td ALIGN=left>$row[0]</td></tr>";
			$INFOout .= "<tr><th ALIGN=right>$label_vendor_lead_code: &nbsp; </th><td ALIGN=left>$row[1]</td></tr>";
			$INFOout .= "<tr><th ALIGN=right>List ID: &nbsp; </th><td ALIGN=left>$row[2]</td></tr>";
			$INFOout .= "<tr><th ALIGN=right>Timezone: &nbsp; </th><td ALIGN=left>$row[3]</td></tr>";
			$INFOout .= "<tr><th ALIGN=right>Called Since Last Reset: &nbsp; </th><td ALIGN=left>$row[4]</td></tr>";
			$INFOout .= "<tr><th ALIGN=right>$label_phone_code: &nbsp; </th><td ALIGN=left>$row[5]</td></tr>";
			$INFOout .= "<tr><th ALIGN=right>$label_phone_number: &nbsp; </th><td ALIGN=left>$row[6] - &nbsp; &nbsp; &nbsp; &nbsp; ";
			if ($hide_dial_links < 1) {$INFOout .= "<a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG',$row[5], $row[6], $lead_id);return false;\">DIAL</a>";
			}
			$INFOout .= "</td></tr>";
			$INFOout .= "<tr><th ALIGN=right>$label_title: &nbsp; </th><td ALIGN=left>$row[7]</td></tr>";
			$INFOout .= "<tr><th ALIGN=right>$label_first_name: &nbsp; </th><td ALIGN=left>$row[8]</td></tr>";
			$INFOout .= "<tr><th ALIGN=right>$label_middle_initial: &nbsp; </th><td ALIGN=left>$row[9]</td></tr>";
			$INFOout .= "<tr><th ALIGN=right>$label_last_name: &nbsp; </th><td ALIGN=left>$row[10]</td></tr>";
			$INFOout .= "<tr><th ALIGN=right>$label_address1: &nbsp; </th><td ALIGN=left>$row[11]</td></tr>";
			$INFOout .= "<tr><th ALIGN=right>$label_address2: &nbsp; </th><td ALIGN=left>$row[12]</td></tr>";
			$INFOout .= "<tr><th ALIGN=right>$label_address3: &nbsp; </th><td ALIGN=left>$row[13]</td></tr>";
			$INFOout .= "<tr><th ALIGN=right>$label_city: &nbsp; </th><td ALIGN=left>$row[14]</td></tr>";
			$INFOout .= "<tr><th ALIGN=right>$label_state: &nbsp; </th><td ALIGN=left>$row[15]</td></tr>";
			$INFOout .= "<tr><th ALIGN=right>$label_province: &nbsp; </th><td ALIGN=left>$row[16]</td></tr>";
			$INFOout .= "<tr><th ALIGN=right>$label_postal_code: &nbsp; </th><td ALIGN=left>$row[17]</td></tr>";
			$INFOout .= "<tr><th ALIGN=right>Country: &nbsp; </th><td ALIGN=left>$row[18]</td></tr>";
			$INFOout .= "<tr><th ALIGN=right>$label_gender: &nbsp; </th><td ALIGN=left>$row[19]</td></tr>";
			$INFOout .= "<tr><th ALIGN=right>$label_alt_phone: &nbsp; </th><td ALIGN=left>$row[20] - &nbsp; &nbsp; &nbsp; &nbsp; ";
			if ($hide_dial_links < 1) {$INFOout .= "<a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG',$row[5], $row[20], $lead_id, 'ALT');return false;\">DIAL</a>";
			}
			$INFOout .= "</td></tr>";
			$INFOout .= "<tr bgcolor=white><th ALIGN=right>$label_email: &nbsp; </th><td ALIGN=left>$row[21]</td></tr>";
			$INFOout .= "<tr bgcolor=white><th ALIGN=right>$label_security_phrase: &nbsp; </th><td ALIGN=left>$row[22]</td></tr>";
			$INFOout .= "<tr bgcolor=white><th ALIGN=right>$label_comments: &nbsp; </th><td ALIGN=left>$row[23]</td></tr>";
			$INFOout .= "<tr bgcolor=white><th ALIGN=right>Called Count: &nbsp; </th><td ALIGN=left>$row[24]</td></tr>";
			$INFOout .= "<tr bgcolor=white><th ALIGN=right>Last Local Call Time: &nbsp; </th><td ALIGN=left>$row[25]</td></tr>";
					...
					if (strlen($select_SQL) > 0) {
						...
						if ($list_lead_ct > 0) {
							...
							while ($fields_to_print > $o) {
								...
								$INFOout .= "<tr bgcolor=white><td ALIGN=right>$A_field_name[$o]: &nbsp; </td><td ALIGN=left>$A_field_value</td></tr>";
...
*/
		...
		$NOTESout = '';
		if ($search != 'logfirst') {$NOTESout .= "<h4>Histórico da Lead:</h4><DIV  class=datagrid >\n";
		}
		$NOTESout .= "<TABLE>";
		$NOTESout .= "<thead><TR>";
		$NOTESout .= "<TH>#</TH>";
		$NOTESout .= "<TH>Data</TH>";
		$NOTESout .= "<TH>Operador</TH>";
		$NOTESout .= "<TH>Tempo (seg.)</TH>";
		$NOTESout .= "<TH>Estado</TH>";
		$NOTESout .= "<TH>Nº</TH>";
		$NOTESout .= "<TH>Campanha</TH>";
		$NOTESout .= "<TH title='Entrada Ou Saida'>IO</TH>";
		$NOTESout .= "<TH>Nº Alt</TH>";
		$NOTESout .= "<TH>Desligou</TH>";
		#	$NOTESout .= "</TR><TR>";
		...
		if ($g > 0) {sort($ALLsort, SORT_NUMERIC);
		} else {$NOTESout .= "<tr><td colspan=11 align=center>No calls found</td></tr>";
		}
		...
		while ($g > $u) {
			...
			$NOTESout .= "<td align=right>$ALLcall_date[$i]</td>";
			$NOTESout .= "<td align=right> $ALLuser[$i]</td>\n";
			$NOTESout .= "<td align=right> $ALLlength_in_sec[$i]</td>\n";
			$NOTESout .= "<td align=right> $ALLstatus[$i]</td>\n";
			$NOTESout .= "<td align=right> $ALLphone_number[$i] </td>\n";
			$NOTESout .= "<td align=right> $ALLcampaign_id[$i] </td>\n";
			$NOTESout .= "<td align=right> $ALLin_out[$i] </td>\n";
			$NOTESout .= "<td align=right> $ALLalt_dial[$i] </td>\n";
			$NOTESout .= "<td align=right> $ALLhangup_reason[$i] </td>\n";
			/*$NOTESout .= "</TR><TR>";
			$NOTESout .= "<td></td>";
		...
		if ($search == 'logfirst') {echo "$NOTESout\n$INFOout\n";
		} else {echo "$INFOout\n$NOTESout\n";
		}
...
if ($ACTION == 'CALLSINQUEUEgrab') {
	$stmt = "SELECT view_calls_in_queue,grab_calls_in_queue from vicidial_campaigns where campaign_id='$campaign'";
	if ($non_latin > 0) {$rslt = mysql_query("SET NAMES 'UTF8'") OR die(mysql_error());
	}
...
if ($ACTION == 'CalLBacKLisT') {
	if ($agentonly_callback_campaign_lock > 0) {$campaignCBsql = "and a.campaign_id='$campaign'";
	} else {$campaignCBsql = '';
	}
	...
	while ($callbacks_count > $loop_count) {
		...
		//echo "$row[0] ~$row[1] ~$row[2] ~$callback_id[$loop_count] ~$lead_id[$loop_count] ~$campaign_id[$loop_count] ~$status[$loop_count] ~$entry_time[$loop_count] ~$callback_time[$loop_count] ~$comentario\n";
		$callback[] = array("name"=>$row[0]." ".$row[1],"phone"=>$row[2],"callback_id"=>$callback_id[$loop_count],"lead_id"=>$lead_id[$loop_count],"campaign_id"=>$campaign_id[$loop_count],"status"=>$estado,"entry_time"=>$entry_time[$loop_count],"callback_time"=>$callback_time[$loop_count],"comment"=>$comentario);
                $loop_count++;
	}
...
if ($ACTION == 'CalLBacKCounT') {
	...
	echo json_encode(array($cbcount,$cbcount_live));
}
...
if ($ACTION == 'get_tempo_pausa') {
	...
	$smt = "SELECT pause_code_name, max_time from vicidial_pause_codes WHERE campaign_id = '$campaign' AND max_time > 0";
...
FROM
    vicidial_agent_log a
        INNER JOIN
    vicidial_pause_codes b ON a.sub_status = b.pause_code
WHERE
    b.campaign_id = '$campaign' AND pause_code_name = '$nowPausa' AND event_time between CURDATE() AND NOW() AND user = '$user'";
        $result=mysql_query($query,$link);
	$result = mysql_fetch_assoc($result);
	$itv = number_format($result['tempo'] / 60, 0);
	$max = number_format($ze / 60, 0);
	if ($itv > $max) {
		$itv = "<span align='center' style=color:red>" . ($max - $itv) ." minutos </span>";
	} else {
		$itv = "<span align='center'>" . ($max - $itv) . " minutos </span>";
	}

	echo "<tr>
			<td height='35px'>$nowPausa</td>
			<td height='35px'>
			" . $itv ."
			</td>
			</tr>
...
if ($ACTION == 'pesquisa_morada'){
	...
	$rsl=mysql_query("SELECT * FROM `cp7` WHERE `cod_postal` LIKE '".mysql_real_escape_string($cp_4)."-".mysql_real_escape_string($cp_3)."' Limit 250;", $link);
		
		if (mysql_num_rows($rsl)==0) {
			echo "Não teve resultados.";
		}
		else{
			echo "
			<table> \n
			...
			";
		for ($i=0; $i < mysql_num_rows($rsl) ; $i++) {
				$linha=mysql_fetch_assoc($rsl); 
			echo "
				<tr>
					<td>$linha[rua]</td>
					<td>$linha[cod_postal]</td>
					<td>$linha[localidade]</td>
                                        <td>$linha[freguesia]</td>
                                        <td>$linha[concelho]</td>    
					<td>$linha[distrito]</td> 
					<td><img src='../images/icons/map_add_32.png' alt='Abrir' style='cursor:pointer;vertical-align:middle;' onclick=\"aplica_morada('".$linha[rua]."', '".$linha[cod_postal]."', '".$linha[localidade]."', '".$linha[freguesia]."', '".$linha[concelho]."', '".$linha[distrito]."')\"></td>
			</tr>";
		}
