...
function custom_list_fields_values($lead_id,$list_id,$uniqueid,$user)
	...
	if ($tablecount_to_print > 0) 
		...
		if ($fieldscount_to_print > 0) 
			...
			if ($fields_to_print < 1) 
				{$CFoutput .= "<tr bgcolor=white><td colspan=4><font size=1>There are no custom fields for this list</td></tr>";}
			...
			while ($fields_to_print > $o) 
				...
				if ($last_field_rank=="$A_field_rank[$o]")
					...
					{
					$CFoutput .= "</td></tr>\n";
					$CFoutput .= "<tr bgcolor=white><td  align="; #id=td_$A_field_label[$o] $new_action[$o]
				...
				if ( ($A_name_position[$o]=='TOP') or ($A_field_type[$o]=='SCRIPT') )
					{/*$CFoutput .= "<span style=\"position:static;\" id=P_HELP_$A_field_label[$o]></span><span style=\"position:static;background:white;\" id=HELP_$A_field_label[$o]> $helpHTML</span>";*/}
				else
					{
					if ($last_field_rank=="$A_field_rank[$o]")
						{$CFoutput .= "";} 
					else
						{$CFoutput .= "</td><td align=left>";}
					}
				$field_HTML='';

				if ($A_field_type[$o]=='SELECT')
					{
					$field_HTML .= "<select class='select-required' size=1 name=$A_field_label[$o] id=$A_field_label[$o] $new_action[$o]>\n";
					}
				if ($A_field_type[$o]=='MULTI')
					{
					$field_HTML .= "<select class='multiple-required".$A_field_label[$o]."' MULTIPLE style='height:90px; width:375px;' size=$A_field_size[$o] $new_action[$o] name=$A_field_label[$o][] id=$A_field_label[$o][]>\n"; #$new_action[$o]
					}
					...
					while ($te < $field_options_count)
						...
						if (preg_match("/,/",$field_options_array[$te]))
							...
							if ( ($A_field_type[$o]=='RADIO') or ($A_field_type[$o]=='CHECKBOX') )
								...
								$field_HTML .= "<input class='radiocheck-required'  type=$A_field_type[$o] name=$A_field_label[$o][] id=$A_field_label[$o][] value=\"$field_options_value_array[0]\" $field_selected> $field_options_value_array[1]\n"; #$new_action[$o]
								if ($A_multi_position[$o]=='VERTICAL') 
									{$field_HTML .= "\n";}
								}
							}
				...
				if ($A_field_type[$o]=='TEXT') 
					...
					$field_HTML .= "<input class='selectmulti-required' $new_action[$o] type=text size=$A_field_size[$o] maxlength=$A_field_max[$o] name=$A_field_label[$o] id=$A_field_label[$o] value=\"$A_field_value[$o]\">\n";
					}
				if ($A_field_type[$o]=='AREA') 
					{
					if ($A_field_default[$o]=='NULL') {$A_field_default[$o]='';}
					if (strlen($A_field_value[$o]) < 1) {$A_field_value[$o] = $A_field_default[$o];}
					$field_HTML .= "<textarea name=$A_field_label[$o] id=$A_field_label[$o] ROWS=$A_field_max[$o] COLS=10 style='width:400px'>$A_field_value[$o]</textarea>";
					}
				...
				if ($A_field_type[$o]=='DATE') 
					...
					$field_HTML .= "<input class='date-required' readonly='readonly' type=text size=11 maxlength=10 name=$A_field_label[$o] id=$A_field_label[$o] value='' >\n";
					
					if($new_action[$o] != 'AMOVERRIDE'){
				...
				if ($A_field_type[$o]=='TIME') 
					...
					$field_HTML .= "<SELECT class='hour-required' $new_action[$o] name=HOUR_$A_field_label[$o] id=HOUR_$A_field_label[$o]>";
				...
				/*	$field_HTML .= "<option>21</option>";
					...
					$field_HTML .= "</SELECT>";
					$field_HTML .= "<SELECT class='minute-required' name=MINUTE_$A_field_label[$o] id=MINUTE_$A_field_label[$o]>";
					$field_HTML .= "<option>---</option>";
                    $field_HTML .= "<option>00</option>";
					$field_HTML .= "<option>05</option>";
					$field_HTML .= "<option>10</option>";
	...
	if (preg_match("/--A--/",$CFoutput))
		...
		$stmt="SELECT lead_id,entry_date,modify_date,status,user,vendor_lead_code,source_id,list_id,gmt_offset_now,called_since_last_reset,phone_code,phone_number,title,first_name,middle_initial,last_name,address1,address2,address3,city,state,province,postal_code,country_code,gender,date_of_birth,alt_phone,email,security_phrase,comments,called_count,last_local_call_time,rank,owner,extra1,extra2,extra3,extra4,extra5,extra6,extra7,extra8,extra9,extra10,extra11,extra12,extra13,extra14,extra15 FROM vicidial_list where lead_id='$lead_id' LIMIT 1;";
		$rslt=mysql_query($stmt, $link);
		...
		$CFoutput = eregi_replace('--A--extra1--B--',"$extra1",$CFoutput);
		$CFoutput = eregi_replace('--A--extra2--B--',"$extra2",$CFoutput); 
		$CFoutput = eregi_replace('--A--extra3--B--',"$extra3",$CFoutput);
		$CFoutput = eregi_replace('--A--extra4--B--',"$extra4",$CFoutput);
		$CFoutput = eregi_replace('--A--extra5--B--',"$extra5",$CFoutput);
		$CFoutput = eregi_replace('--A--extra6--B--',"$extra6",$CFoutput);
		$CFoutput = eregi_replace('--A--extra7--B--',"$extra7",$CFoutput);
		$CFoutput = eregi_replace('--A--extra8--B--',"$extra8",$CFoutput);
		$CFoutput = eregi_replace('--A--extra9--B--',"$extra9",$CFoutput);
		$CFoutput = eregi_replace('--A--extra10--B--',"$extra10",$CFoutput);
		$CFoutput = eregi_replace('--A--extra11--B--',"$extra11",$CFoutput);
		$CFoutput = eregi_replace('--A--extra12--B--',"$extra12",$CFoutput);
		$CFoutput = eregi_replace('--A--extra13--B--',"$extra13",$CFoutput);
		$CFoutput = eregi_replace('--A--extra14--B--',"$extra14",$CFoutput);
		$CFoutput = eregi_replace('--A--extra15--B--',"$extra15",$CFoutput);
