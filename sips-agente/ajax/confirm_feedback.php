...
switch ($action) {
    ...
    case "validate_call":
        ...
        $query = "SELECT max(`uniqueid`)  from vicidial_log where lead_id='$lead_id' and user='$user'";

        $query = mysql_query($query, $link) or die(mysql_error());
        $row = mysql_fetch_assoc($query);
        $uniqueid = $row["uniqueid"];

        $query = "INSERT INTO `crm_confirm_feedback`(`id`, `lead_id`,`uniqueid`,  `feedback`, `sale`, `campaign`, `agent`, `comment`,date,admin) VALUES (NULL,$lead_id,'$uniqueid','validado por agente',0,'$campaign','$user','$comment','" . date('Y-m-d H:i:s') . "','Inexistente')";
        $query = mysql_query($query, $link) or die(mysql_error());
