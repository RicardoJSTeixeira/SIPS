...
if( (strlen($user)<2) or (strlen($pass)<2) or ($auth==0))
	...
	if( (strlen($server_ip)<6) or (!isset($server_ip)) or ( (strlen($session_name)<12) or (!isset($session_name)) ) )
		{
		echo json_encode(array("Invalid_server_ip"=>$server_ip,"Invalid_session_name"=> $session_name));
		exit;
		}
	else
		{
		...
		$SNauth=$row[0];
		  if($SNauth==0)
			{
			echo json_encode(array("Invalid_session_name"=>array($session_name,$server_ip)));
			exit;
			}
		  else
			{
...
echo "<BODY BGCOLOR=white marginheight=0 marginwidth=0 leftmargin=0 topmargin=0>\n";
	...
	if ($ACTION == 'refresh')
	{
		...
		if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'03008',$user,$server_ip,$session_name,$one_mysql_log);}
			...
			if ( ($api_manual_dial=='QUEUE') or ($api_manual_dial=='QUEUE_AND_AUTOCALL') )
				...
				if ( ($MDQ_count > 0) and (strlen($external_dial) < 16) and ($Astatus=='PAUSED') and ($Alead_id < 1) )
					...
					if ($mdq_record_ct > 0)
                        ...
                        if (strlen($external_status)<1) {$external_status = explode(":",'::::::::::');}else{$external_status=explode(":",$external_status);}
			...
			$InGroupChangeDetails = explode("|",'0|||');
			...
			if ($manager_ingroup_set > 0)
				...
				if ($VLAMISaffected_rows_update > 0)
					...
					if ($migs_record_ct > 0)
						...
						$InGroupChangeDetails = explode("|","1|$external_blended|$external_igb_set_user|$external_igb_set_name");
	...
	#	echo "$channels_list|";
		...
		{
			$counter++;
			$countecho[] = $ChannelA[$counter];
		#	echo "$ChannelA[$counter] ~";
		}

	//echo "$countecho\n";
        echo json_encode(array(array("DateTime"=>$NOW_TIME,"UnixTime"=>$StarTtime,"Logged"=>$Alogin,"CampCalls"=>$RingCalls,"Status"=>$Astatus,"DiaLCalls"=>$DiaLCalls,"APIHanguP"=>$external_hangup,"APIStatuS"=>$external_status,"APIPausE"=>$external_status,"APIPausE"=>$external_pause,"APIDiaL"=>$external_dial,"DEADcall"=>$DEADcustomer,"InGroupChange"=>$InGroupChangeDetails,"APIFields"=>$external_update_fields,"APIFieldsData"=>$external_update_fields_data,"APITimerAction"=>$timer_action,"APITimerMessage"=>$timer_action_message,"APITimerSeconds"=>$timer_action_seconds,"APIdtmf"=>$external_dtmf,"APItransferconf"=>$external_transferconf,"APIpark"=>$external_park,"APITimerDestination"=>$timer_action_destination,"APIManualDialQueue"=>$MDQ_count),array($channels_list,$countecho)));
