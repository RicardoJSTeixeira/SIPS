...
if($action == "last_outbound")
{
if($apenas_campanha){ $filtro_campanha = " AND vclog.campaign_id='$campaign_id' ";} else { $filtro_campanha = "  "; }	
if($chm_auto){ $filtro_chm = " AND vclog.comments='AUTO' ";} else { $filtro_chm = " AND vclog.comments='MANUAL' "; }	
...
while ( $aRow = mysql_fetch_array( $rResult ) )
{
	...
	switch($aColumns[$i])
	{
		case "first_name": $row[] = "<a onclick='LoadCRMEdit($aRow[lead_id]);' style='cursor:pointer'> ".$aRow['first_name']."<img title='Ver Detalhes' style='float:right' src='../../images/icons/livejournal_16.png'></a>"; break;
		case "phone_number" : $row[] = "<a onclick='???($aRow[lead_id]);' style='cursor:pointer'> ".$aRow['phone_number']."<img title='Chamar' style='float:right' src='../../images/icons/telephone_go_16.png'></a>"; break;
...
if ($action == 'update_feedback') {
	...
	if($querya and $queryb and $queryc and $queryd){ } else {echo mysql_query("ROLLBACK"); echo "Erro ao Gravar na DB.";}
