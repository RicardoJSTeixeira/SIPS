...
if (isset($_GET["in_group_id"]))			{$id_group_inb=$_GET["in_group_id"];}
	elseif (isset($_POST["in_group_id"]))	{$id_group_inb=$_POST["in_group_id"];}

if ($id_group_inb!=""){$inbound=1;}
...
if ( (strlen($user)<2) or (strlen($pass)<2) or ($auth==0) or ( ($LVAactive < 1) and ($VUmodify < 1) ) )
	...
	{
	#quando corre bem faz coisas
	}
     
if ($inbound>0) { $list_id = $id_group_inb;  }
	...
	if ($tablecount_to_print > 0) 
		...
		while ($fields_to_print > $o) 
			...
			if ( ($A_field_type[$o]=='DISPLAY') or ($A_field_type[$o]=='SCRIPT') )
				...
				$SUBMIT_output .= "<tr style='border:1px solid #c0c0c0'><td align=left>$A_field_name[$o]</td><td align=left style='word-wrap:break-word'>$A_field_value[$o]</td></tr>";
				}
			$o++;
			}
...
### END parse submission of the custom fields form ###
	...
	echo "		{\n";
	echo "		document.getElementById(\"P_\" + taskspan).innerHTML = \" &nbsp; <a href=\\\"javascript:close_help('\" + taskspan + \"','\" + taskhelp + \"');\\\">Ajuda-</a><BR> &nbsp; \";\n";
	...
	echo "		document.getElementById(taskspan).innerHTML = \" &nbsp; <a href=\\\"javascript:open_help('\" + taskspan + \"','\" + taskhelp + \"');\\\">Ajuda+</a>\";\n";
