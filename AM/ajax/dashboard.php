...
switch ($action) {
   ...
   where c.user_group=:user_group and DATE(a.start_date)>'2014-07-21' and a.gone=0  group by a.lead_id limit 20000";
   ...
   where a.id_resource in ('" . implode("','", $refs) . "') and DATE(a.start_date)>'2014-07-21' and a.gone=0 group by a.lead_id limit 20000";
        }
    ...
    where c.user_group=:user_group and a.list_id=:list and (d.lead_id is null or d.gone=0) and a.extra6='NO' and DATE(d.start_date)>'2014-07-21' limit 20000";
            $variables[":user_group"] = $u->user_group;
        } else {
            $query = "SELECT
                CONCAT(b.first_name, ' ', b.middle_initial, ' ', b.last_name),
    ...
    inner join `vicidial_list` b on a.extra7=b.lead_id 
    left join sips_sd_reservations c on a.lead_id=c.lead_id
    where c.id_user in ('" . implode("','", $u->siblings) . "') and a.list_id=:list and (c.lead_id is null or c.gone=0) and a.extra6='NO' and DATE(c.start_date)>'2014-07-21' limit 20000";
        }
   ...
   inner join vicidial_list b on a.lead_id=b.lead_id 
   left join spice_consulta c on c.reserva_id=a.id_reservation
   where a.id_resource in ($refs) and a.end_date<:date and c.id is NULL and DATE(a.start_date)>'2014-07-21' and a.gone=0 
   UNION ALL
   ...
   inner join vicidial_list b on a.lead_id=b.lead_id 
   left join spice_consulta c on c.reserva_id=a.id_reservation
   where a.id_resource in ($refs) and a.end_date<:date1 and c.closed=0 and DATE(a.start_date)>'2014-07-21' and a.gone=0 ) a order by a.end_date asc";
