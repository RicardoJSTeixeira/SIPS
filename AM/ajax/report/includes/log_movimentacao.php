...
$query_log = "SELECT a.user,a.entry_date,a.data,a.status,b.event_date,b.record_id,b.type,b.note FROM spice_report_movimentacao a inner join (select max(id),record_id, type,note,event_date,section from  spice_log  where  section='Mov. Stock' group by record_id ) b on a.id=b.record_id where  a.entry_date  BETWEEN :data_inicial AND :data_final;";
