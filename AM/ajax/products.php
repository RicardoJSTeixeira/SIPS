...
switch ($action) {
    ...
    case "apagar_produto_by_id":
        ...
        $log->set($id, Logger::T_RM, Logger::S_PROD, json_encode(array("product" => $products->get_product_name($id))),logger::A_DECL_);
        break;
    ...
    case "edit_product":
        ...
        $log->set($id, Logger::T_UPD, Logger::S_PROD, json_encode(array("product" => $name)),logger::A_NCHANGE);
        break;

    case "add_promotion":
        $produto = new product($db, $id);
        $last_insert_id = $produto->add_promotion($active, $highlight, $data_inicio, $data_fim);
        $log->set($last_insert_id, Logger::T_INS, Logger::S_PROM, json_encode(array("type" => "Criar Promoção", "active" => $active, "highlight" => $highlight, "data_inicio" => $data_inicio, "data_fim" => $data_fim, "produto" => $products->get_product_name($id))),logger::A_APV);
    ...
    case "remove_promotion":
        ...
        $log->set($id_promotion, Logger::T_RM, Logger::S_PROM, json_encode(array("type" => "Remover Promoção", "produto" => $products->get_product_name($id))),logger::A_DECL);
        break;
