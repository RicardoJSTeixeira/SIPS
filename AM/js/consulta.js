...
$("#print_proposta").click(function() {
    ...
    if ($("#proposta_form").validationEngine("validate"))
    {
        ...
        doc.addImage(image, 'JPEG', 55, 55, 200, 65);
        doc.setFontSize(13);
        doc.text(60, y.pos(135), 'Refª Cliente: ' + client_box.get_info().refClient);
        doc.setFontSize(12);
        ...
        doc.text(325, y.y, (p1.valor * (p1.entrada / 100)).toFixed(2) + '€');
        doc.text(240 - (p1.meses.length * 7), y.pos(), p1.meses + ' Prestações ');
        doc.text(325, y.y, (p1.valor - (p1.valor * (p1.entrada / 100)) / p1.meses).toFixed(2) + '€');
        if (p2.valor) {
            doc.setFontSize(14);
            ...
            doc.text(305, y.y, (p2.valor * (p2.entrada / 100)).toFixed(2) + '€');
            doc.text(240 - (p2.meses.length * 7), y.pos(), p2.meses + ' Prestações ');
            doc.text(305, y.y, (p2.valor - (p2.valor * (p2.entrada / 100)) / p2.meses).toFixed(2) + '€');
        }
        if (p3.valor) {
            doc.setFontSize(14);
            ...
            doc.text(305, y.y, (p3.valor * (p3.entrada / 100)).toFixed(2) + '€');
            doc.text(240 - (p3.meses.length * 7), y.pos(), p3.meses + ' Prestações ');
            doc.text(305, y.y, (p3.valor - (p3.valor * (p3.entrada / 100)) / p3.meses).toFixed(2) + '€');
        }
        ...
        doc.setFontSize(11);
     /*  doc.text(60, y.pos(20), 'A presente proposta tem validade durante o mês corrente, devendo fora deste prazo ser');
        doc.text(60, y.pos(), 'confirmada com o nosso audioprotesista:');
        doc.text(80, y.pos(10), 'Ludgero Lopes - 912 741 803');
*/
