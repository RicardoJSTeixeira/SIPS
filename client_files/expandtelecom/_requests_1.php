...
if($action == "get_email_list")
{
    ...
    $sQuery = "SELECT sent_by_campaign, sent_by_user, event_date, sent_to_client, sent_to_email, sent_to_result FROM email_log WHERE event_date between '$sent_data 00:00:00' AND '$sent_data 23:59:59'";
    //echo $sQuery;
...
if($action == "get_sales_list")
{
	...
	$comercias="";
	/*if($user=="fseixas"){
	  $comercias=" AND d.id_resource='3' ";
	  }elseif($user=="jpedrogao"){
	  $comercias=" AND d.id_resource='1' ";
	  }*/
	...
	$sQuery = "SELECT a.lead_id, b.full_name, c.first_name, a.phone_number, a.call_date FROM vicidial_log a INNER JOIN vicidial_users b ON a.user=b.user INNER JOIN vicidial_list c ON a.lead_id=c.lead_id INNER JOIN sips_sd_reservations d on a.lead_id=d.lead_id WHERE a.campaign_id IN ($sent_campaign_id) AND a.status='VENDA' AND d.start_date between '$sent_data 00:00:00' AND '$sent_data 23:59:59' $comercias group by d.lead_id";
