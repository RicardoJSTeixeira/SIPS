...
$script_query = (($tem_Script == 1) ? "INNER JOIN custom_$campanha_id B ON A.lead_id = B.lead_id" : "");
$stmt="SELECT C.call_date AS 'Data da Chamada', source_id AS 'Tipo da Chamada', $campos, vcs.status_name as 'Feedback' FROM vicidial_list A $script_query INNER JOIN (SELECT status,status_name,campaign_id FROM vicidial_campaign_statuses UNION ALL SELECT status,status_name,'BANANA' FROM vicidial_statuses) vcs ON vcs.status = A.status INNER JOIN (SELECT call_date, lead_id FROM vicidial_log group by lead_id order by call_date DESC) AS C ON C.lead_id=B.lead_id WHERE modify_date BETWEEN '$query_date' AND '$end_date' $feedbacks_str AND (vcs.campaign_id = '$campanha_id' OR vcs.campaign_id = 'BANANA')";
