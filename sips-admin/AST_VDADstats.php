...
echo "<div class=cc-mstyle>";
	echo "<table>";
	echo "<tr>";
	echo "<td id='icon32'><img src='../images/icons/chart_pie_32.png' /></td>";
	echo "<td id='submenu-title'> Estatísticas Outbound </td>";
...
echo "<tr>";
echo "<td valign=top><label>Data Inicio</label> <input type=text style='width:145px; text-align:center; text:indent:0;' id=sd name=query_date value=\"$query_date\">";


echo "<label>Data Final</label> <input type=text  style='width:145px; text-align:center; text:indent:0;' id=ed name=end_date  value=\"$end_date\">";
...
#echo "<input type=text style='border:none; width:85px;' disabled=disabled> </td>";
echo "<td valign=top><label> Campanhas </label>";
echo "<select style='width:300px; height:120px;' name=group[] multiple>";
if  (eregi("--ALL--",$group_string))
	{echo "<option value=\"--ALL--\" selected>  --Todas as Campanhas-- </option>\n";}
else
	{echo "<option value=\"--ALL--\"> --Todas as Campanhas-- </option>\n";}
$o=0;
while ($campaigns_to_print > $o)
	{
	if (eregi("$groups[$o]\|",$group_string)) {echo "<option selected value=\"$groups[$o]\">$group_names[$o]</option>\n";}
	  else {echo "<option value=\"$groups[$o]\">$group_names[$o]</option>\n";}
	$o++;
	}
echo "</select>";
...
/*echo "<label>Incluir <i>Drop Rollover</i></label>";
echo "<SELECT style=outline:none NAME=include_rollover>\n";

$selected = "";
$selected1 = "";

if ($include_rollover == "YES") { $selected = "selected";} else { $selected1 = "selected"; }


#echo "<option selected value=\"$include_rollover\">$include_rollover</option>\n";
echo "<option $selected value=\"YES\">Sim </option>";
echo "<option $selected1 value=\"NO\">Não</option>";

echo "</SELECT>\n"; 

/*echo "<label>Bottom Graph </label>";
...
if ($carrier_stats == "YES") { $selected = "selected";} else { $selected1 = "selected"; }
	
	
	echo "<label>Incluir Estados de <i>Carrier</i> </label>";
...
case "ALL" : $selected = "selected"; break; 
case "AM" : $selected1 = "selected"; break;
case "PM" : $selected2 = "selected"; break;
...
echo "<option $selected value=\"ALL\">Todos</option>\n";
echo "<option $selected1 value=\"AM\">Manhã</option>\n";
echo "<option $selected2 value=\"PM\">Tarde</option>\n";
...
#echo "<tr><td align=center colspan=3><INPUT type=submit class=styled-button style=float:none; NAME=SUBMIT VALUE=SUBMIT></td></tr>";
...
/*	$OUToutput .= "Productivity Rating:                          $AVG_ANSWERagent_non_pause_sec\n";
	...
	<tr>
	<td><b> Total de Chamadas para esta Campanha(s): </td>
	<td> $TOTALcalls </td>
	</tr>
	
	<tr>
	<td><b> Tempo Médio de Chamada: </td>
	<td>  $average_call_seconds </td>
	</tr>	
	...
	<h9> Respostas Humanas </h9>
	<div class=cc-mstyle>
	<table style='width:30%'>
	
	<tr>
	<td> <b>Total de Respostas Humanas: </td>
	<td> $CIcalls </td>
	</tr>
	
	<tr>
	<td><b>Tempo Médio de Chamada: </td>
	<td>  $average_ci_seconds </td>
	</tr>	
	...
	<h9> Chamadas Perdidas </h9>
	<div class=cc-mstyle>
	<table style='width:70%'>
	
	<tr>
	<td><b> Total de Chamadas Perdidas em Outbound </td>
	<td> $DROPcalls  |  $DROPpercent% </td>
	</tr>
	
	<tr>
	<td><b>Percentagem de Chamadas Perdidas <i> versus </i> Chamadas Atendidas: </td>
	<td>  $DROPcalls / $ANSWERcalls  |  $DROPANSWERpercent% </td>
	</tr>	
	
	<tr>
	<td><b> Percentagem de Chamadas Perdidas <i> versus </i> Chamadas Atendidas com <i>Rollback</i>: </td>
	<td>  $DROPcalls / $CIcallsRAW | $inDROPANSWERpercent% </td>
	</tr>	
	
	<tr>
	<td><b> Tempo médio de Chamadas Perdidas: </td>
	<td>  $average_hold_seconds </td>
	</tr>	
	
	<tr>
	<td><b> Rácio de Produção:  </td>
	<td> $AVG_ANSWERagent_non_pause_sec </td>
	</tr>
	...
	<tr>
	<td><b> Total de Chamadas sem Resposta (Busy, Disconnect, RingNoAnswer) </td>
	<td> $totalNAcalls  |  $NApercent% </td>
	</tr>
	
	<tr>
	<td><b>Total de Chamadas Não Atendidas (Modo Automático): </td>
	<td> $autoNAcalls </td>
	</tr>	
	
	<tr>
	<td><b> Total de Chamadas Não Atendidas (Modo Manual): </td>
	<td>   $manualNAcalls </td>
	</tr>	
	
	<tr>
	<td><b>Tempo Médio das Chamadas Não Atendidas</td>
	<td>  $average_na_seconds </td>
	</tr>	
...
/*	$OUToutput .= "\n";
	...
	print "
	<h9> Estatísticas de <i>Hangup</i> </h9>
	<div class=cc-mstyle>
	<table style='width:60%; margin-top:-36px'>
	
	<tr>
	<th> Motivo de <i>Hangup</i> </th>
	<th> Chamadas </th>
	</tr>
	...
	while ($i < $reasons_to_print)
		...
		if (ereg("NONE",$reason))	{$reason = 'Sem Resposta';}
		if (ereg("CALLER",$reason)) {$reason = 'Cliente';}
		if (ereg("AGENT",$reason)) {$reason = 'Operador';}
		if (ereg("ABANDON",$reason)) {$reason = 'Chamada Abandonada';}
		if (ereg("QUEUETIMEOUT",$reason)) {$reason = 'Tempo em Fila Excedido';}
		$OUToutput .= "| $reason | $REASONcount |\n";
		
		print "<tr><td> $reason </td><td> $REASONcount </td> </tr>";
...
/*	$OUToutput .= "+----------------------+------------+\n";
	...
	$OUToutput .= "+----------------------+------------+\n"; */
		...
		echo "<tr><th></th></tr>";
		echo "<tr style='border-top:2px solid black'> <td> TOTAL: </td> <td> $TOTALcalls </td>";
		echo "<tr><th></th></tr>";
		echo "</table>";
		echo "</div>";
...
##########################################################################################################################################################################################################################################################################################################################################
	...
	print "
	<h9>  Estatísticas de Feedback </h9>
	<div class=cc-mstyle>
	<table style='width:95%; margin-top:-36px;'>
	...
	<th> Tempo de Chamadas do Agente por Hora </th>
	</tr>
...
/*	$OUToutput .= "\n";
	...
	while ($i < $statuses_to_print)
		...
		print "<tr><!--<td> $status </td>--><td> $status_name </td><td> $STATUScount </td><td> $STATUShours </td><td> $STATUSavg </td><td> $STATUSrate </td><td> $AGENTrate </td></tr>";
...
#	$OUToutput .= "|   AGENT TIME                                                      | $aTOTALhours |                     | $aTOTALrate |\n";
	...
	echo "<tr><th></th></tr>";
	echo "<tr style='border-top:2px solid black'> <td colspan=3> TOTAL: </td> <td> $TOTALcalls </td><td> $TOTALhours </td><td> $TOTALavg </td><td> $TOTALrate </td><td></td>";
	echo "<tr><th></th></tr>";
	echo "</table>";
	echo "</div>";
...
/*	$OUToutput .= "\n";
	...
	<h9> Estatísticas das Bases de Dados </h9>
	<div class=cc-mstyle>
	<table style='width:80%; margin-top:-32px;'>
	...
	while ($i < $listids_to_print)
		...
		$LISTIDname =	sprintf("%-40s", "$LISTIDlist_names[$i]");#while(strlen($LISTIDname)>40) {$LISTIDname = substr("$LISTIDname", 0, -1);}

		/* $OUToutput .= "| $LISTIDname | $LISTIDcount |\n"; */
		print "<tr><td> $LISTIDname </td><td> $LISTIDcount </td> </tr>";
		$i++;
		}
...
/*	print "
	...
	<h9> Estatísticas dos Agentes </h9>
	<div class=cc-mstyle>
	<table style='width:80%; margin-top:-36px'>
	<tr>
	<th> Agente </th>
	<th> Chamadas </th>
	<th> Tempo Total de Chamadas </th>
...
/*		print "<tr><td> $full_name </td><td> $USERcalls </td> <td> $USERtotTALK_MS </td> <td> $USERavgTALK_MS </td></tr>";
...
/*	echo "<tr><th></th></tr>";
	echo "<tr style='border-top:2px solid black'> <td> Total de Agentes: $TOTagents </td> <td> $TOTcalls </td> <td> $TOTtime </td> <td> $TOTavg </td> </tr>";
	echo "<tr> <td> Tempo Médio de Espera </td> <td></td> <td></td> <td> $AVGwait </td>";
	echo "<tr><th></th></tr>";
	echo "</table>";
	echo "</div>";*/
