...
#	http://1.1.1.1/goautodial-admin/recording_lookup.php
#	 - user/pass: VDC
#
...
#	http://1.1.1.1/goautodial-admin/recording_lookup_DIRECT.php?phone=7275551212&format=GSM&auth=VDC1234593JH654398722
#
...
#
### remove temp files
# 1 7 * * * /usr/bin/find /usr/local/apache2/htdocs/goautodial-admin/temp/ -maxdepth 1 -type f -mtime +1 -print | xargs rm -f
#
...
$US='_';
  ...
  if(eregi("VDC1234593JH654398722",$auth))
    ...
    exit;
		...
		$fp = fopen ("/usr/local/apache2/htdocs/goautodial-admin/auth_entries.txt", "a");
		$date = date("r");
...
if ( (strlen($phone)<10) or (strlen($phone)>10) ) 
	...
	if ($logs_to_print)
		...
		if ($format == 'WAV')
			{
			exec("/usr/local/apache2/htdocs/goautodial-admin/wget --output-document=/usr/local/apache2/htdocs/goautodial-admin/temp/$AUDname[$AUDnamect] $location\n");

			$AUDIOfile = "/usr/local/apache2/htdocs/goautodial-admin/temp/$AUDname[$AUDnamect]";
		...
		if ($format == 'GSM')
			{
			passthru("/usr/local/apache2/htdocs/goautodial-admin/wget --output-document=/usr/local/apache2/htdocs/goautodial-admin/temp/$fileGSM $locationGSM\n");

			$AUDIOfile = "/usr/local/apache2/htdocs/goautodial-admin/temp/$fileGSM";
