        ...
        if (strlen($dialstatuses) > 1) {
            ...
            while ($print_ctp < $ctp) {
                ...
                switch ($TFhour_status[$print_ctp]) {
                    ...
                    case "ANSWER" : $TFhour_status[$print_ctp] = "Respostas";
                        break;
                    case "BUSY" : $TFhour_status[$print_ctp] = "Ocupado";
                        break;
                    case "CANCEL" : $TFhour_status[$print_ctp] = "Canceladas";
                        break;
                    case "CHANUNAVAIL" : $TFhour_status[$print_ctp] = "Sem Canal Disponível";
                        break;
                    case "CONGESTION" : $TFhour_status[$print_ctp] = "Marcações com Erro";
                        break;
                    case "NOANSWER" : $TFhour_status[$print_ctp] = "Sem Resposta";
                        break;
                }
...
######################################################################		
        ...
        switch ($DIALmethod) {
            case "MANUAL": $DIALmethod = "Manual";
                break;
            case "RATIO": $DIALmethod = "Rácio";
                break;
            case "ADAPT_HARD_LIMIT": $DIALmethod = "Adapt. Limite";
                break;
            case "ADAPT_TAPERED": $DIALmethod = "Adapt. Alterada";
                break;
            case "ADAPT_AVERAGE": $DIALmethod = "Adapt. Média";
                break;
            case "INBOUND_MAN": $DIALmethod = "Inbound";
                break;
        }
        ...
        switch ($DIALorder) {
            ...
            case "RANDOM": $DIALorder = "Aleatória";
                break;
            case "DOWN": $DIALorder = "Descendente";
                break;
            case "UP": $DIALorder = "Ascendente";
                break;
            case "DOWN PHONE": $DIALorder = "Tlf. Descendente";
                break;
            case "UP PHONE": $DIALorder = "Tlf. Ascendente";
                break;
            case "DOWN LAST NAME": $DIALorder = "Ult. Nome Descendente";
                break;
            case "UP LAST NAME": $DIALorder = "Ult. Nome Ascendente";
                break;
        }
...
if ($UidORname > 0) {
    if ($RTajax > 0) {
        $HTuser = " <th><a href='#' onclick=\"update_variables('orderby','user');\">Operador</a><img src=/images/icons/user_16.png onclick=\"update_variables('UidORname','');\" /></th>     ";
    } #<img src=/images/icons/google_adsense_16.png onclick=\"javascript:ingroup_info('$Luser','$j');\" />
    else {
    ...
    if ($RTajax > 0) {
        $HTuser = " <th><a href='#' onclick=\"update_variables('orderby','user');\">Operador</a><img src=/images/icons/user_gray_16.png onclick=\"update_variables('UidORname','');\" /></th>     ";
    } #<img src=/images/icons/google_adsense_16.png onclick=\"javascript:ingroup_info('$Luser','$j');\" />
    else {
        $HTuser = " <th><a href='../sips-admin/$PHP_SELF?$groupQS&RR=$RR&DB=$DB&adastats=$adastats&SIPmonitorLINK=$SIPmonitorLINK&IAXmonitorLINK=$IAXmonitorLINK&usergroup=$usergroup&UGdisplay=$UGdisplay&UidORname=$UidORname&orderby=$userord&SERVdisplay=$SERVdisplay&CALLSdisplay=$CALLSdisplay&PHONEdisplay=$PHONEdisplay&CUSTPHONEdisplay=$CUSTPHONEdisplay&with_inbound=$with_inbound&monitor_active=$monitor_active&monitor_phone=$monitor_phone&ALLINGROUPstats=$ALLINGROUPstats&DROPINGROUPstats=$DROPINGROUPstats&NOLEADSalert=$NOLEADSalert&CARRIERstats=$CARRIERstats&PRESETstats=$PRESETstats&AGENTtimeSTATS=$AGENTtimeSTATS\">Operador</a></th>  ";
    }
...
if ((eregi('ALL-ACTIVE', $group_string)) and (strlen($group_SQL) < 3)) {
    $UgroupSQL = '';
} else {
...
### Run through the loop to display agents
    ...
    $stmtC = "select pause_code,pause_code_name from vicidial_pause_codes";
    $rsltC = mysql_query($stmtC, $link);

    for ($i=0;$i<mysql_num_rows($rsltC);$i++) {
        $rowC = mysql_fetch_row($rsltC);
        $newPauseCodes[$rowC[0]] = $rowC[1];
    }
    ...
    while ($j < $talking_to_print) {
        ...
        if (($Lstatus == 'INCALL') or ($Lstatus == 'PARK')) {
            if ($call_time_S >= 10) {
                $G = '<PRE class="thistle"><B>';
                $EG = '</B></PRE>';
            }
            if ($call_time_S >= 60) {
                $G = '<PRE class="violet"><B>';
                $EG = '</B></PRE>';
            }
            if ($call_time_S >= 300) {
                $G = '<PRE class="purple"><B>';
                $EG = '</B></PRE>';
            }
        ...
        if ($Lstatus == 'PAUSED') {
            if (!ereg("N", $agent_pause_codes_active)) {
                ...
                $stmtC = "select sub_status from vicidial_agent_log where agent_log_id >= '$Aagent_log_id[$i]' and user='$Luser' limit 1;";
                $rsltC = mysql_query($stmtC, $link);
                $rowC = mysql_fetch_row($rsltC);
                $pausecode = $newPauseCodes[$rowC[0]];
            } else {
                $pausecode = '';
            }
        ...
        if ((eregi("PARK", $status))) {
            $agent_ready++;
        }
        if ((eregi("READY", $status)) or (eregi("CLOSER", $status))) {

            $G = '<PRE class="lightblue"><B>';
            $EG = '</B></PRE>';
            if ($call_time_S >= 60) {
                $G = '<PRE class="blue"><B>';
                $EG = '</B></PRE>';
        ...
        $R = '';
        if ($SIPmonitorLINK > 0) {
            $L = "<td><a href=\"sip:0$Lsessionid@$server_ip\">LISTEN</a>";
            $R = '';
        }
        if ($IAXmonitorLINK > 0) {
            $L = "<td><a href=\"iax:0$Lsessionid@$server_ip\">LISTEN</a>";
            $R = '';
        }
        if ($SIPmonitorLINK > 1) {
            $R = "<td><a href=\"sip:$Lsessionid@$server_ip\">BARGE</a>";
        }
        if ($IAXmonitorLINK > 1) {
            $R = "<td><a href=\"iax:$Lsessionid@$server_ip\">BARGE</a>";
        }



        if ((strlen($monitor_phone) > 1) and (preg_match("/MONITOR|BARGE/", $monitor_active) )) {
            $L = "<td> <a href=\"javascript:send_monitor('$Lsessionid','$Lserver_ip','MONITOR');\"><img src='/images/icons/sound_add_16.png' /></a>";
            $R = '';
        }
        ...
        if ($SERVdisplay > 0) {
            $SVD = " <td>$G$server_ip$EG</td> <td>$G$call_server_ip$EG</td>";
        } else {
            $SVD = "";
        }

        if ($PHONEdisplay > 0) {
            $phoneD = "<td>$G$phone$EG</td> ";
        } else {
            $phoneD = "";
        }
        ...
        if ($campaign_allow_inbound == '1') {
            $HTigcall .= "<th>IN-GROUP</th>";
        }
        ...
        if ($realtime_block_user_info < 1) {
            ...
            if (($status == 'PAUSED') && ($query_r[0] == 'RATIO')) {
                $force_var = "<span style=\"cursor:pointer;\" onclick=\"ForceReady('$Auser[$i]','" . trim($user) . "')\"><img title='Forçar Ready' src='/images/icons/user_go_16.png'></span>";
            } else {
                $force_var = "<span><img title='Não é possivel Forçar READY neste User. \n - O Operador tem de estar PAUSED. \n - A Campanha não pode ser MANUAL.' src='/images/icons/user_delete_16.png'></span>";
            }
            ...
            if (strpos($inb_users, $Luser) !== false) {
                $inb_flag = 'style=background-color:#99CCFF';
            } else {
                $inb_flag = 'teste';
            }
            $Aecho .= "<tr>
