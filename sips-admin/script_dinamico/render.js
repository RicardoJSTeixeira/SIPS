var render = function(script_zone, file_path, script_id, lead_id, unique_id, user_id, campaign_id, admin_review, ext_events) {
    ...
    function before_all(callback) {
        if (me.lead_id) {
            $.post(file_path + "requests.php", {
                action: "get_client_info_by_lead_id",
                lead_id: me.lead_id,
                user_logged: user_id
            },
            function(info1) {
    ...
    function update_script(callback) {
        if (me.script_id !== undefined) {
            $.post(file_path + "requests.php", {
                action: "get_scripts_by_id_script",
                id_script: me.script_id
            },
            function(data) {
                if (Object.size(data)) {
                    if (data !== null) {
            ...
            $.post(file_path + "requests.php", {
                action: "get_render_scripts_by_campaign",
                id_campaign: camp_linha,
                lead_id: me.lead_id
            },
            function(data) {
                if (Object.size(data)) {
        ...
        $.each(script_zone.find("#script_div .pag_div .item"), function() {
            ...
            switch (info.type) {
                case "texto":
                    ...
                    if (info.default_value)
                        if (info.default_value.name != 0 && Object.size(me.client_info) && !me.admin_review) {
                            if (info.default_value.name == "lead_id")
                                input.value = me.lead_id;
                            else
                ...
                case "datepicker":
                    ...
                    element.find(".form_datetime").data("format", options.format_rules);
                ...
                case "scheduler":
                    ...
                    $.post(file_path + "requests.php", {
                        action: "get_schedule_by_id",
                        ids: info.values_text.join(",")
                    },
                    function(info3) {
    ...
    this.populate_script = function(callback) {
        $.post(file_path + "requests.php", {
            action: "get_results_to_populate",
            search_spice: me.config.save_overwrite,
            lead_id: me.lead_id,
            id_script: me.script_id,
    ...
    this.submit_manual = function(callback) {
        $.post(file_path + "requests.php", {
            ...
            "save_overwrite": me.config.save_overwrite,
            id_script: me.script_id,
            results: $("#script_form").serializeArray(),
            user_id: me.user_id,
            unique_id: me.unique_id,
            campaign_id: me.campaign_id,
            lead_id: me.lead_id,
            admin_review: me.admin_review
        },
        function() {
            admin_review = 0;
            if (typeof callback === "function") {
