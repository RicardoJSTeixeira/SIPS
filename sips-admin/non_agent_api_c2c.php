...
if (isset($_GET["newdiallevel"]))			{$new_dial_level=$_GET["newdiallevel"];}
	elseif (isset($_POST["newdiallevel"]))	{$new_dial_level=$_POST["newdiallevel"];}
...
################################################################################
### alterar o racio de marcação
################################################################################

if ($function == 'change_dial_level') 
{
		
/*	$query = mysql_query("SELECT user_group FROM vicidial_users WHERE user='$_SERVER[PHP_AUTH_USER]'", $link);
	...
	{
	echo "Não é possível alterar o Rácio de Marcação a todas as Campanhas. Por favor escolha só a Campanha que pretende modificar."; exit;
	}

	if (!preg_match("/^\d*\.?\d*$/",$new_dial_level) || $new_dial_level < 1)
	{ 
	echo "Não foi possivel alterar o Rácio de Marcação. \n\n - O Rácio de Marcação só aceita números inteiros. \n\n - O Rácio de Marcação não pode ser inferior a 1."; exit;
	}		
	
	mysql_query("UPDATE vicidial_remote_agents SET number_of_lines='$new_dial_level' WHERE campaign_id='$campaign_id'", $link);
