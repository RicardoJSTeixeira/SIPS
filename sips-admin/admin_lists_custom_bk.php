...
$NWB = " &nbsp; <a style='float:right; padding:0;' href=\"javascript:openNewWindow('$PHP_SELF?action=HELP";
...
if ($action == "COPY_FIELDS_FORM")
	...
	echo "<tr>";
	echo "<td> COPIAR CAMPOS PERSONALIZADOS </td>";
	echo "</tr>";	
	echo "</table>"; 
	...
	if ($list_id=="$source_list_id")
		...
		if ($source_field_exists < 1)
			...
			if ($copy_option=='REPLACE')
				...
				if ($table_exists > 0)
					{
					$stmt="SELECT field_id,field_label from vicidial_lists_fields where list_id='$list_id' order by field_rank,field_order,field_label;"; #sql
					$rslt=mysql_query($stmt, $link);
...
/**	echo "<table id='sub-menu'>";
	echo "<tr>";
	#echo "<td> EDITAR CAMPOS PERSONALIZADOS </td>";
...
if ($navigation == "resumo") {
	...
	echo "<td id='submenu-title'> Campos Personalizados </td>";
	echo "<td style='text-align:left'>Inserir Texto.</td>";
	echo "<td id=icon32><a href=\"$PHP_SELF?action=MODIFY_CUSTOM_FIELDS&list_id=$list_id&navigation=novo\"> <img src='/images/icons/script_edit.png' /></a></td>";
...
/*	if ($fields_to_print < 1) 
		{echo "<tr><td colspan=5> Esta lista não contem campos personalizados. </td></tr>";}
	else
		{
...
$stmt="SELECT field_id,field_label,field_name,field_description,field_rank,field_help,field_type,field_options,field_size,field_max,field_default,field_cost,field_required,multi_position,name_position,field_order from vicidial_lists_fields where list_id='$list_id' AND field_label='$id_campo' order by field_rank,field_order,field_label;";
	$rslt=mysql_query($stmt, $link);
	$fields_to_print = mysql_num_rows($rslt);
	$fields_list='';
	$o=0;
	...
	echo "<td id='submenu-title'> Editar Campo Personalizado </td>";
	...
	while ($fields_to_print > $o) 
		...
		echo "</select><td style=width:30px>$NWB#vicidial_lists_fields-field_rank$NWE \n";
		...
		echo "<tr><td>Conteudo do Campo</td><td colspan=4><textarea style='height:125px' name=field_name rows=2 cols=60>$A_field_name[$o]</textarea><td style=width:30px;> $NWB#vicidial_lists_fields-field_name$NWE </td></tr>\n";
		...
		echo "<a style='top:16px; position:relative;' href=\"$PHP_SELF?action=DELETE_CUSTOM_FIELD_CONFIRMATION&list_id=$list_id&field_id=$A_field_id[$o]&field_label=$A_field_label[$o]&DB=$DB\"> Apagar este Campo </a>";
...
{	
	...
	echo "<tr>";
	echo "<td id='icon32'><img src='/images/icons/database_table.png' /></td>";
	echo "<td id='submenu-title'> Adicionar Novo Campo Personalizado </td>";
	echo "<td style='text-align:left'></td>";
	echo "</tr>";
	echo "</table>";
	echo "</div>";
	...
	echo "<tr><td style='min-width:175px'> <div class=cc-mstyle style='height:28px;  '><p> ID do Campo</p></div></td><td colspan=4><input type=text name=field_label style=width:300px maxlength=50><td style=width:30px;> $NWB#vicidial_lists_fields-field_label$NWE </td></tr>";
	...
	echo "<tr><td style='min-width:175px'> <div class=cc-mstyle style='height:28px;  '><p>Conteudo do Campo</p></div></td><td colspan=4><textarea style=height:125px name=field_name rows=2 cols=60></textarea><td style=width:30px;> $NWB#vicidial_lists_fields-field_name$NWE </td></tr>\n";
	...
	echo "<tr><td style='min-width:175px'> <div class=cc-mstyle style='height:28px;  '><p>Opções do Campo</p></div></td><td colspan=4><textarea style=height:125px name=field_options></textarea><td style=width:30px;>  $NWB#vicidial_lists_fields-field_options$NWE </td></tr>\n";
...
if ($action == "LIST")
	...
	echo "<tr>";
	echo "<td id='icon32'><img src='/images/icons/script_edit.png' /></td>";
	echo "<td id='submenu-title'> Campos Costumizados </td>";
	echo "<td style='text-align:left'>Inserir Texto.</td>";
	echo "</tr>";
	echo "</table>";
	echo "</div>";
	...
	echo "<th> Nome da DB </th>";
	echo "<th> Activa </th>";
	echo "<th> Campanha </th>\n";
	echo "<th> Campos Costumizados </th>\n";
	echo "<th> Editar </th>\n";
	echo "</TR>\n";
	...
	while ($lists_to_print > $o) 
		...
		#echo "<tr><td><font size=1><a href=\"admin.php?ADD=311&list_id=$A_list_id[$o]\">$A_list_id[$o]</a></td>";
		echo "<td> $A_list_name[$o]</td>";
		echo "<td> $A_active[$o]</td>";
		echo "<td> $A_campaign_id[$o]</td>";
		echo "<td> $A_list_fields_count[$o]</td>";
		echo "<td><a href=\"$PHP_SELF?action=MODIFY_CUSTOM_FIELDS&list_id=$A_list_id[$o]&navigation=resumo\"><img src='/images/icons/livejournal.png' /></a></td></tr>\n";
