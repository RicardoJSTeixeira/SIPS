    ...
    if ($Flag == "ALL" || $Flag == "DIALOG") {
        ...
        if (mysql_num_rows($query)) {
            ...
            foreach ($AllFields as $index => $value) {
                switch ($value) {
                    case "FIRST_NAME": $active = 1;
                        $displayname = "Nome";
                        $order = 1;
                        break;
                    case "PHONE_NUMBER": $active = 1;
                        $displayname = "Telefone";
                        $order = 2;
                        break;
                    case "ALT_PHONE": $active = 1;
                        $displayname = "Telefone Alternativo";
                        $order = 3;
                        break;
                    case "ADDRESS3": $active = 1;
                        $displayname = "Telemóvel";
                        $order = 4;
                        break;
                    case "ADDRESS1": $active = 1;
                        $displayname = "Morada";
                        $order = 5;
                        break;
                    case "POSTAL_CODE": $active = 1;
...
switch ($action) {
    case "FieldsListBuilder": FieldsListBuilder($CampaignID, $AllFields, $FieldID, $FieldDisplayName, $FieldReadOnly, $Flag, $link);
        break;
    case "FieldsReadOnlySwitch": FieldsReadOnlySwitch($CampaignID, $FieldID, $ReadOnly, $link);
        break;
    case "ReOrderFields": ReOrderFields($CampaignID, $SortedFields, $link);
        break;
    case "RemoveField": RemoveField($CampaignID, $FieldID, $link);
        break;
    case "DialogFieldsEditOnSave": DialogFieldsEditOnSave($CampaignID, $FieldID, $FieldName, $link);
        break;
    case "DialogFieldsApplyToAllCampaignsOnSave": DialogFieldsApplyToAllCampaignsOnSave($CampaignID, $AllowedCampaigns, $link);
        break;
    case "DialogFieldsCopyOnOpen": DialogFieldsCopyOnOpen($CampaignID, $ModAllowedCampaigns, $link);
        break;
    case "BtnCopyFields": BtnCopyFields($CampaignID, $CopyCampaignID, $link);
        break;
}