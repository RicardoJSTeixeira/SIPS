    ...
    if ($Flag == "ALL" || $Flag == "REFRESH") {
        ...
        if (mysql_num_rows($query) > 0) {
            while ($row = mysql_fetch_assoc($query)) {
                ...
                if ($row['sysname'] == NULL) {
                    $js['recycle_name'][] = $row['campname'];
                } else {
                    $js['recycle_name'][] = $row['sysname'];
                }
            }
        }
    ...
    while ($aRow = mysql_fetch_array($rResult)) {
        ...
        for ($i = 0; $i < count($aColumns); $i++) {
            switch ($aRow['tentativas']) {
                case "N": $index = 0;
                    $try_type = "Não Chamado";
                    break;
                case "Y": $index = 1;
                    $try_type = "Chamado";
                    break;
                default: $explode = explode("Y", $aRow['tentativas']);
                    $index = $explode[1] + 1;
                    $try_type = $explode[1] . "ª Reciclagem";
                    break;
            }
            switch ($aColumns[$i]) {
                case "tentativas": $row[] = $try_type;
                    break;
                case "contactos": $row[] = $aRow[$aColumns[$i]];
                    break;
                case "reset": if ($index == 0) {
                        $cursor = "style='cursor: default !important;'";
                    } else {
                        $cursor = "";
                    } $row[] = "<img $cursor class='css-dt-icon recycle-details-reset-single' title='Realiza um Reset aos Contactos que têm este número de tentativas.' src='icons/mono_undo_16.png'>";
                    break;
                case "detalhes": $row[] = "<img class='css-dt-icon recycle-details-view-contacts' inner-try-type='$aRow[tentativas]' title='Ver Contactos' src='icons/mono_right_expand_16.png'>";
                    break;
                case "index": $row[] = $index;
                    break;
...
function DialogEditRecycleOnSave($RecycleID, $RecycleDelay, $RecycleTries, $CampaignID, $link) {
      global $user;
    $RecycleDelay = ($RecycleDelay * 60);
    $query1 = "UPDATE vicidial_lead_recycle SET attempt_delay = '$RecycleDelay', attempt_maximum='$RecycleTries' WHERE campaign_id='$CampaignID' AND status='$RecycleID'";
    ...
    $query = "Insert into vicidial_admin_log(`admin_log_id`, `event_date`, `user`, `ip_address`, `event_section`, `event_type`, `record_id`, `event_code`, `event_sql`)"
            . "values(NULL,'" . date("Y-m-d H:i:s") . "','" . $user->id . "','" . $user->ip . "','RECYCLE','MODIFY','$CampaignID','ADMIN EDIT RECYCLE','" . mysql_real_escape_string($query1) . "')";
...
function EditRecycleResetSingleTries($RecycleID, $Index, $CampaignLists, $link) {
    ...
    foreach ($CampaignLists as $value) {
        ...
        $query = "Insert into vicidial_admin_log(`admin_log_id`, `event_date`, `user`, `ip_address`, `event_section`, `event_type`, `record_id`, `event_code`, `event_sql`)"
                . "values(NULL,'" . date("Y-m-d H:i:s") . "','" . $user->id . "','" . $user->ip . "','RECYCLE','RESET','$value','ADMIN RESET RECYCLE SINGLE TRIES','" . mysql_real_escape_string($query1) . "')";
        mysql_query($query) or die(mysql_error());
    }
}
...
function EditRecycleResetAllTries($RecycleID, $CampaignLists, $link) {
    ...
    foreach ($CampaignLists as $value) {
        ...
        $query = "Insert into vicidial_admin_log(`admin_log_id`, `event_date`, `user`, `ip_address`, `event_section`, `event_type`, `record_id`, `event_code`, `event_sql`)"
                . "values(NULL,'" . date("Y-m-d H:i:s") . "','" . $user->id . "','" . $user->ip . "','RECYLCE','RESET','$value','ADMIN RESET RECYCLE ALL TRIES','" . mysql_real_escape_string($query1) . "')";
        mysql_query($query) or die(mysql_error());
    }
}
...
function TableRecycleContactDetailsInit($RecycleID, $TryType, $CampaignLists, $link) {
    ...
    while ($aRow = mysql_fetch_array($rResult)) {
        ...
        for ($i = 0; $i < count($aColumns); $i++) {
            switch ($aColumns[$i]) {
                case "first_name": $row[] = $aRow['first_name'];
                    break;
                case "phone_number": $row[] = $aRow['phone_number'];
                    break;
                case "total_calls": $row[] = $aRow['total_calls'];
                    break;
                case "last_call": $row[] = $aRow['last_call'];
                    break;
                case "reset": $row[] = "<img class='css-dt-icon recycle-contact-details-single-reset' lead-id='$aRow[lead_id]' title='Realiza um Reset a este Contacto.' src='icons/mono_undo_16.png'>";
                    break;
                case "dnc": $row[] = "<img class='css-dt-icon recycle-contact-details-single-disable' lead-id='$aRow[lead_id]' title='Retira este contacto da Reciclagem.' src='icons/mono_minus_16.png'>";
                    break;
                default: $row[] = $aRow[$aColumns[$i]];
            }
        }
...
function DialogRecycleResetCallbacksOnSave($CampaignID, $link) {
    ...
    while ($row = mysql_fetch_assoc($query)) {
        ...
        mysql_query("UPDATE vicidial_list SET status='NEW', user='', called_since_last_reset ='N' WHERE lead_id='$row[lead_id]'", $link) or die(mysql_error());
...
switch ($action) {
    case "RecycleListBuilder": RecycleListBuilder($CampaignID, $CampaignLists, $RecycleID, $RecycleName, $RecycleDelay, $RecycleTries, $AllowedCampaigns, $Flag, $link);
        break;
    case "RecycleAvailFeeds": RecycleAvailFeeds($CampaignID, $link);
        break;
    case "RecycleActiveInactive": RecycleActiveInactive($CampaignID, $RecycleID, $RecycleActive, $NumRecycle, $link);
        break;
    case "ApplyRecycleToAllCampaings": ApplyRecycleToAllCampaings($RecycleID, $RecycleActive, $RecycleDelay, $RecycleTries, $link);
        break;
    case "GetRecycleDetails": GetRecycleDetails($CampaignID, $CampaignLists, $RecycleID, $link);
        break;
    case "DialogEditRecycleOnSave": DialogEditRecycleOnSave($RecycleID, $RecycleDelay, $RecycleTries, $CampaignID, $link);
        break;
    case "EditRecycleResetSingleTries": EditRecycleResetSingleTries($RecycleID, $Index, $CampaignLists, $link);
        break;
    case "EditRecycleResetAllTries": EditRecycleResetAllTries($RecycleID, $CampaignLists, $link);
        break;
    case "TableRecycleContactDetailsInit": TableRecycleContactDetailsInit($RecycleID, $TryType, $CampaignLists, $link);
        break;
    case "EditRecycleContactDetailsResetAll": EditRecycleContactDetailsResetAll($RecycleID, $TryType, $CampaignLists, $link);
        break;
    case "EditRecycleContactDetailsDisableAll": EditRecycleContactDetailsDisableAll($RecycleID, $TryType, $CampaignLists, $link);
        break;
    case "EditRecycleContactDetailsResetSingle": EditRecycleContactDetailsResetSingle($LeadID, $link);
        break;
    case "EditRecycleContactDetailsDisableSingle": EditRecycleContactDetailsDisableSingle($LeadID, $link);
        break;
    case "DialogRecycleResetCallbacksOnSave": DialogRecycleResetCallbacksOnSave($CampaignID, $link);
        break;
}
