...
var newModTable;
	...
	if((Flag == "ALL" && $("#tbl-fields tr").length == 0) || Flag == "NEW" || Flag == "DIALOG")
	{
		...
		if(Flag == "DIALOG") { newModCampaign = ModCampaign; newModTable = "#tbl-fields-copy";  $("#tbl-fields-copy").empty();  } else { newModCampaign = CampaignID; newModTable = "#tbl-fields";  }
		...
		$.ajax({
            ...
            data: { action: "FieldsListBuilder",
					...
					FieldReadOnly : function(){ if($("#input-new-field-readonly").parent().hasClass()) return 0; else return 1; },
					Flag: Flag
                    },
            success: function(json) 
			{
				...
				if(Flag == "NEW") { AvailFields.splice(0, 1); $("#input-new-field").val("") }
				
				
				$.each(json.name, function(index, value){
					
					if(Flag == "ALL") $.each(AvailFields, function(index1, value1){ if(value == value1){ AvailFields.splice(index1, 1) } })
					...
					if(json.readonly[index]==0) readonlyChecked = "checked='checked'"; else readonlyChecked = "";
					if(Flag == "NEW"){ if($("#input-new-field-readonly").parent().hasClass("checked")){ readonlyChecked = "checked='checked'" } else { readonlyChecked = ""; } }
...
function FieldsElemInit()
{
	...
	$("#tbl-fields").sortable({
		stop: function( event, ui ) {OddEvenRefresh("", "tbl-fields"); ReOrderFields(); },
...
function FieldsReadOnlySwitch()
{
	...
	if($(this).parent().hasClass("checked")) ReadOnly = 0; else ReadOnly = 1;
	...
	$.ajax({
            ...
            success: function(json) 
			{
				...
				$.each(json.c_id, function(index, value){
					...
					if(index == 0){ FieldsListBuilder("DIALOG", value); }
...
$.each(sortedIDs, function(index, value){
    ...
    if($("#ro-"+value).attr("src")=="icons/mono_document_16.png"){sortedReadOnly.push(1)} else {sortedReadOnly.push(0)};
...
$('#new-field').click(function(){
    ...
    $(".rdonly-chooser div span").each(function(){
        ...
        if($(this).hasClass("checked") && $(this).children().attr("id")=="new-field-rdonly-no"){ new_field_readonly = "icons/mono_document_empty_16.png"; return false; } else { new_field_readonly = "icons/mono_document_16.png"; return false; }
...
$('.icon-readonly').live("click", function(){
    if($(this).attr("src")=="icons/mono_document_16.png"){$(this).attr("src", "icons/mono_document_empty_16.png")} else { $(this).attr("src", "icons/mono_document_16.png") }
    })
