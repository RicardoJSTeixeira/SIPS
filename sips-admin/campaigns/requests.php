...
if ($action == "dT_campaign-monitor") {
    ...
    while ($aRow = mysql_fetch_array($rResult)) {
        ...
        for ($i = 0; $i < count($aColumns); $i++) {
            switch ($aColumns[$i]) {
                case "campaign_name": {
                        if ($aRow['pauses'] == 0 || $aRow['feedbacks'] == 0 || $aRow['dynamic_fields'] == 0) {
                            $camp_warning_text = "style='color:red'";
                            $camp_warning = "<img class='mono-icon' title='Campanha com erros na configuração, por favor edite a Campanha e corrija estes erros.' src='icons/mono_alert_16.png'>";
                        } else {
                            $camp_warning = "";
                            $camp_warning_text = "";
                        } $row[] = "<div style='float:left'><span $camp_warning_text>$aRow[campaign_name]</div><div  style='float:left; top:-3px'>$camp_warning</div></span>";
                        break;
                    }
                case "campaign_id": {
                        $row[] = "<img id='img-campaign-edit' class='icon pointer' html-campaign-id='$aRow[campaign_id]' src='/images/icons/mono_wrench_16.png'>";
                        break;
                    }
                case "dial_method": if ($aRow[$aColumns[$i]] == "RATIO") {
                        $row[] = "Automática";
                    } else {
                        $row[] = "Manual";
                    };
                    break;
                case "auto_dial_level": {
                        $row[] = "x" . $aRow['auto_dial_level'];
                    };
                    break;
                case "active": if ($aRow[$aColumns[$i]] == "Y") {
                        $row[] = "<img title='Clique para alterar.' class='elem-pointer' id='cmp-enabler-" . $aRow['campaign_id'] . "' onclick=\"CampaignEnabler('$aRow[campaign_id]');\" src='/images/icons/tick_16.png'>";
                    } else {
                        $row[] = "<img title='Clique para alterar.' class='elem-pointer' onclick=\"CampaignEnabler('$aRow[campaign_id]');\" id='cmp-enabler-" . $aRow['campaign_id'] . "' src='/images/icons/cross_16.png'>";
                    };
                    break;
                case "pauses": {
                        if ($aRow[$aColumns[$i]] == 0) {
                            $row[] = "<b><span style='color:red'>Não</span></b>";
                        } else {
                            $row[] = $aRow[$aColumns[$i]];
                        } break;
                    }
                ...
                case "script": {
                        $query = mysql_query("SELECT campaign_id FROM vicidial_lists_fields WHERE campaign_id='$aRow[campaign_id]'") or die(mysql_query());
                ...
                case "fields": {
                        if ($aRow['dynamic_fields'] > 0) {
                            $row[] = $aRow['dynamic_fields'];
                        } else {
                            $row[] = "<b><span style='color:red'>Não</span></b>";
                        } break;
                    }
                case "recycle": {
                        $user_recycle = $aRow[$aColumns[$i]] - 7;
                        $row[] = "<b>7</b> + " . $user_recycle;
                        break;
                    }
                case "db-count": {
