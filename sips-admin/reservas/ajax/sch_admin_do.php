...
$end_time=preg_replace($only_nr, '' , $_POST[end_time]);
	...
	$query="INSERT INTO `sips_sd_schedulers` (`display_text`, `alias_code`, `days_visible`, `blocks`, `begin_time`, `end_time`, `active`)
			VALUES
	...
	mysql_query($query,$link) or die(__json_encode(array(sucess=>1))."Sucedeu-se um Erro. ".mysql_error());   
	echo __json_encode(array(sucess=>1,id=>mysql_insert_id(),display_text=>$display_text,alias_code=>$alias_code,display_days=>$display_days,blocks=>m2h($blocks),begin_time=>m2h($begin_time,true),end_time=>m2h($end_time,true)));                                               
?>                                                                             
