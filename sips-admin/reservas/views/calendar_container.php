    ...
    <body>
        <div class='grid' >
            ...
            if (isset($_GET["sch"]) OR isset($_GET["cp"]) OR isset($_GET["ref"])) {
                $query = "SELECT id_reservation, start_date, end_date, a.id_resource,id_user,a.lead_id,id_reservation_type,b.display_text, d.postal_code FROM sips_sd_reservations a INNER JOIN sips_sd_resources b ON a.id_resource=b.id_resource  INNER JOIN sips_sd_reservations_types c ON a.id_reservation_type=c.id_reservations_types LEFT JOIN vicidial_list d ON a.lead_id = d.lead_id WHERE gone=0 and del=0 and b.id_scheduler=$id_scheduler And ((start_date <='$end_sys' And start_date >='$begin_sys') or (end_date <='$end_sys' And end_date >='$begin_sys') or (start_date <='$end_sys' And end_date >='$begin_sys'));";
            } elseif (isset($_GET["rsc"])) {
                $query = "SELECT id_reservation, start_date, end_date, id_resource,id_user,a.lead_id,id_reservation_type,display_text, c.postal_code FROM sips_sd_reservations a  INNER JOIN sips_sd_reservations_types b ON a.id_reservation_type=b.id_reservations_types LEFT JOIN vicidial_list c ON a.lead_id = c.lead_id WHERE gone=0 and del=0 and id_resource=$id_resource And ((start_date <='$end_sys' And start_date >='$begin_sys') or (end_date <='$end_sys' And end_date >='$begin_sys') or (start_date <='$end_sys' And end_date >='$begin_sys'))";
            }
                ...
                for ($ii = 0; $ii < $resources[0]["days_visible"]; $ii++) {
                    ...
                    for ($i = $resources[0]["begin_time"]; $i <= $resources[0]["end_time"]; $i += $resources[0]["blocks"]) {
                        ...
                        for ($iii = 0; $iii < count($resources); $iii++) {
                            ...
                            $end_hack = date("Y-m-d H:i:s", strtotime($date . "+" . ($i + $resources[0]["blocks"]-1) . " minutes"));
                            $dados = set_estado($beg, $end_hack, $resources[$iii]["id_resource"], $reservas, $series, $resources[$iii]["restrict_days"], $execoes, $date, $i, $slot2change, $i_reservas, $users);
