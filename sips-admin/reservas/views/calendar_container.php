    ...
    <body>
        <div class='grid' >
            ...
            if (isset($_GET["sch"]) OR isset($_GET["cp"]) OR isset($_GET["ref"])) {
                $query = "SELECT id_reservation, start_date, end_date, a.id_resource,id_user,a.lead_id,id_reservation_type,b.display_text, c.postal_code FROM sips_sd_reservations a INNER JOIN sips_sd_resources b ON a.id_resource=b.id_resource  INNER JOIN sips_sd_reservations_types c ON a.id_reservation_type=c.id_reservations_types LEFT JOIN vicidial_list c ON a.lead_id = c.lead_id WHERE b.id_scheduler=$id_scheduler And start_date <='$end_sys' And start_date >='$begin_sys';";
            } elseif (isset($_GET["rsc"])) {
