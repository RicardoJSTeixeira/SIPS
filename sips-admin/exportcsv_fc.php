...
$con = mysql_connect("localhost","sipsadmin","psadm888");
	...
	for($i=0;$i<count($nova_var);$i++) {
			...
			#echo "SELECT * FROM vicidial_list INNER JOIN custom_".$thisCampaign." ON vicidial_list.lead_id = custom_".$thisCampaign.".lead_id WHERE vicidial_list.status IN($feedbacks_IN) AND vicidial_list.list_id IN($listas_IN) AND vicidial_list.last_local_call_time >= '$data_inicial' AND vicidial_list.last_local_call_time <= '$data_final'";
	...
	/*		$feedback_full = mysql_query("SELECT status_name FROM vicidial_campaign_statuses WHERE status='".$row['status']."'", $link);
			...
			if (($row['source_id']==null) || ($row['source_id'])=="") {$campid=$row['comments'];} else {$campid=$row['source_id'];}
			
			if(($campid=="") || ($campid==null)){$campid=$row['extra2'];}

			if($row['owner']=='' || $row['owner']==null){$no=$row['extra1'];} else {$no=$row['owner'];}
			
			switch($row['rastreiimport']){
			case "aud" : $row['rastreiimport']="Audicao"; break;
			case "col" : $row['rastreiimport']="Colestrol"; break;
			case "tens" : $row['rastreiimport']="Tensao Arterial"; break; 
			case "diab" : $row['rastreiimport']="Diabetes"; break; 
			} 
...
/*$query = "	SELECT 	campaign_name
			FROM 	vicidial_campaigns
			WHERE 	campaign_id IN($camps_IN)";
$query = mysql_query($query, $link) or die(mysql_error());

for ($p=0;$p<mysql_num_rows($query);$p++)
{
	$row = mysql_fetch_assoc($query);
	$camp_name[$i] = $row['campaign_name'];
}*/
...
$con = mysql_connect("localhost","sipsadmin","psadm888");
	...
	for($i=0;$i<count($nova_var);$i++) {
			...
			//echo "SELECT * FROM vicidial_list INNER JOIN custom_".$nova_var[$i]." ON vicidial_list.lead_id = custom_".$nova_var[$i].".lead_id WHERE vicidial_list.status IN($feedbacks_IN)  AND vicidial_list.last_local_call_time >= '$data_inicial' AND vicidial_list.last_local_call_time <= '$data_final'";
