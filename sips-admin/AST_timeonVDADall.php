...
/*	if ($ingroups_to_print > 0)
	...
	while ($ingroups_to_print > $r)
		...
		$ingroup_detail .= "<div class='cc-mstyle shad' style='margin-top:8px;'><table>";
		$ingroup_detail .= "<tr>";
...
if ($CARRIERstats > 0)
	...
	#$CARRIERstatsHTML .= "<TR><TD COLSPAN=8>";
	$CARRIERstatsHTML .= "<div style='margin-top:8px;' class=''cc-mstyle shad''>";
	$CARRIERstatsHTML .= "<table >";
	$CARRIERstatsHTML .= "<TR>";
	#$CARRIERstatsHTML .= "<TD>Resumo das Chamadas</TD>";
...
######################################################################		
	echo "<td>";
	echo "<div class='cc-mstyle shad' style='width:100%'>";
	echo "<table>";
	echo "<tr>";
...
#####################################################################	
	...
	echo "<tr>";
	echo "<td id=icon16><img src='../images/icons/chart_curve_16.png'>  <td style='text-align:left'>Chamadas Perdidas / Atendidas:</td><td style='text-align:left'> $dropsTODAY / $answersTODAY  </td>";
	echo "<tr>";
...
######################################################################	
	...
	$AVGwait =		sprintf("%6s", $AVGwait_MS);
		...
		echo "<tr><td id=icon16><img src='/images/icons/clock_go_16.png'>  <td style='text-align:left'>Tempo médio em Chamada:</B></TD><td style='text-align:left'>$avgcustTODAY seg.</TD>";
		echo "<tr><td id=icon16><img src='/images/icons/clock_edit_16.png'>  <td style='text-align:left'>Tempo médio gasto depois de uma Chamada:</TD><td style='text-align:left'>$avgacwTODAY seg.</TD>";
		echo "<tr><td id=icon16><img src='/images/icons/clock_pause_16.png'>  <td style='text-align:left'>Tempo médio em Pausa:</TD><td style='text-align:left'>$avgpauseTODAY seg.</TD>";
...
/*	echo "<div class='cc-mstyle shad' style='margin-bottom:8px;'>";
	echo "<table >";
	echo "<tr>";
	echo "<td> Racio de Marcação: </td> <td>$DIALlev <img style='cursor:pointer' title='Alterar o Rácio de Marcação.' src='../images/icons/livejournal_16.png' onclick=javascript:change_auto_dial_level(\"$group_string\"); /></td>";
	echo "<td> Nivel Máximo: </td><td> $maxDIALlev </td>";
	echo "<td> Chamadas Perdidas: </td><td> $DROPmax% </td>";
	#echo "<td>TARGET DIFF:</td><td> $targetDIFF   </td>";
	#echo "<td> TRUNK SHORT/FILL: </td><td> $balanceSHORT / $balanceFILL </td>";
	#echo "<td> FILTER: </td><td> $DIALfilter </td>";
	#echo "<td> TIME: </td><td> $NOW_TIME </td>";
	echo "</TR>";

/*	if ($adastats>1)
		{
		$min_link="<a href='../sips-admin/$PHP_SELF?$groupQS&RR=4&DB=$DB&adastats=1\">- min </a>";
		if ($RTajax > 0)
			{$min_link='';}
		...
		echo "<td>INTENSITY:</td><td> $ADAintense   </td>";
		echo "</TR>";

		echo "<TR BGCOLOR=\"#CCCCCC\">";
		echo "<td>DIAL TIMEOUT:</td><td> $DIALtimeout </td>";
		echo "<td>TAPER TIME:</td><td> $TAPERtime </td>";
		echo "<td>LOCAL TIME:</td><td> $CALLtime </td>";
		echo "<td>AVAIL ONLY:</td><td> $ADAavailonly </td>";
		echo "</TR>";
		} */

/*	echo "<TR>";
	echo "<td>Contactos Disponiveis:</td><td> $DAleads   </td>";
	
	echo "<td>Média de Colaboradores:</td><td> $agentsONEMIN   </td>";

	echo "</TR>";

	echo "<TR>";
	echo "<td>Hopper ( min/auto ):</td><td> $HOPlev / $AHOPlev   </td>";
	echo "<td>Perdidas / Atendidas:</td><td> $dropsTODAY / $answersTODAY  </td>";
		echo "<td>Metodo de Marcação:</td><td> $DIALmethod   </td>";
	#echo "<td>DL DIFF:</td><td> $diffONEMIN   </td>";
	#echo "<td>STATUSES:</td><td> $DIALstatuses   </td>";
	echo "</TR>";

	echo "<TR>";
	echo "<TD>Contactos no Hopper:</td><td> $VDhop   </td>";
	echo "<TD ALIGN=RIGHT>Percentagem de Chamadas Perdidas:</td><td> ";
	if ($drpctTODAY >= $DROPmax)
		{echo "<font color=red><B>$drpctTODAY%</B>";}
	else
		{echo "$drpctTODAY%";}
	...
	#echo "<td> DIFF: </td><td> $diffpctONEMIN% </td>";
	#echo "<td> ORDER:</td><td> $DIALorder </td>";
	echo "</TR>"; */
...
if ( ($with_inbound != 'O') and ($NOLEADSalert == 'YES') )
	...
	while ($campaigns_to_print > $ctp)
		{
		$row=mysql_fetch_row($rslt);
		$NDLcampaigns .=		" <a href='../sips-admin/./admin.php?ADD=34&campaign_id=$row[0]\">$row[0]</a> ";
		$ctp++;
...
$parked_to_print = mysql_num_rows($rslt);
	...
	/*	echo "<br>";
		echo "<div style='margin-bottom:8px;' class='cc-mstyle shad'>";
		echo "<table>";
...
$Cecho = '<br>';
$Cecho .= "<div class='cc-mstyle shad'><table><tr><th>Status</th><th>Campanha</th><th>Telefone</th><th>Tempo de Dial</th><th>Call type</th><th>Prioridade</th></tr>";
...
while($p<$k)
	...
	$Cecho .= "<tr><td> $G$Cstatus$EG </td><td> $G$Ccampaign_id$EG </td> <td>$G$Cphone_number$EG </td> <td>$G$Ccall_time_MS$EG </td> <td>$G$Ccall_type$EG </td> <td> $G$Cqueue_priority$EG </td> </tr>";
...
$Aecho .= "<div class='cc-mstyle shad'>";
$Aecho .= "<table>";
...
$talking_to_print = mysql_num_rows($rslt);
	...
	$query = mysql_query("SELECT user_group FROM vicidial_users WHERE user='$_SERVER[PHP_AUTH_USER]'") or die(mysql_error());
	
	$query_temp = mysql_fetch_assoc($query);
...
#				{$G='<SPAN class="orange"><B>'; $EG='</B></SPAN>';}
		...
		if ($CUSTPHONEdisplay > 0)	{$CP = " $G<td>$custphone</td>$EG ";}
		else	{$CP = "$G<td>$custphone</td>$EG";}

	/*	if ($UGdisplay > 0)	{$UGD = "<td>$user_group</td>";}
		else	{$UGD = "<td>$user_group</td>";} */
		
		$UGD = "<td>$user_group</td>";

		if ($SERVdisplay > 0)	{$SVD = " $G<td>$server_ip</td>$EG $G<td>$call_server_ip</td>$EG";}
		else	{$SVD = "";}

		if ($PHONEdisplay > 0)	{$phoneD = "$G<td>$phone</td>$EG ";}
		else	{$phoneD = "";}
...
if ($campaign_allow_inbound == '1' ) { $HTigcall .=  "<th>IN-GROUP</th>";    }
		...
		if ($realtime_block_user_info < 1)
			...
			if(($status=='PAUSED') && ($query_r[0]=='RATIO')){$force_var = "<span style=cursor:pointer; onclick=ForceReady('$Auser[$i]')><img title='Forçar Ready' src='../images/icons/user_go_16.png'></span>";}
...
<td>
			...
			<td>
				...
				<div class='cc-mstyle shad' style='margin-bottom:8px; margin-left:-3px; width:100%;'>
				<table>
...
<td>
			...
			<div class='cc-mstyle shad' style='margin-top:8px; margin-bottom:8px; margin-right:-4px; width:95%;'>
			<table>";
			$group_string_alterado = ($group_string=="ALL-ACTIVE") ? "Todos Activos" : $group_string ;
			echo"
			<tr><td id=icon16><img src='../images/icons/events_16.png' /></td> <td style=text-align:left;> Campanha: <b> $group_string_alterado </b> </td> </tr>
			...
			<tr><td id=icon16><img src='../images/icons/database_server_16.png' /></td>	<td style=text-align:left;>Utilização do Servidor: $load_ave</td> </tr>
			</table> 
			</div>
			...
			<div style='margin-bottom:8px; margin-right:-4px; width:95%;' class='cc-mstyle shad'>
			<table>
			$BBB
...
";	
	...
	{
	echo "<center><br><br> Neste momento não se encontram Colaboradores online. werwer </center>";
	echo "<PRE>$Cecho";
