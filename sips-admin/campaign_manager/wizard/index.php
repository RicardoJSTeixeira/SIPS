...
// PAUSES
	...
	for ($i=0;$i<mysql_num_rows($query);$i++)
	{
		...
		$pauses .=  "<tr><td><input type='checkbox'  value='".$row['pause_code']."' name='".$row['pause_code']."' id='".$row['pause_code']."' ><label style='display:inline;' for='".$row['pause_code']."'>".$row['pause_code_name']."</label></td><td style='text-align:right; padding-right:3px;'><span><span id='span-pause-time-".$row['pause_code']."'>".$row['max_time']."</span></span> minutos</td><td class='td-icon'><img id='img-pause-time-".$row['pause_code']."' class='mono-icon edit-pause-time' style='cursor:pointer' src='/images/icons/mono_cup_16.png'></td></tr>"; 
...
// FEEDBACKS
	...
	for ($i=0;$i<mysql_num_rows($query);$i++)
	{
		...
		if($row['human_answered']=="Y"){ $human_answered = "<img class='mono-icon'  src='/images/icons/mono_speech_16.png'>"; } else { $human_answered = ""; }
		if($row['scheduled_callback']=="Y"){ $scheduled_callback = "<img class='mono-icon'  src='/images/icons/mono_phone_16.png'>"; } else { $scheduled_callback = ""; }
		
		if($i>=$half){
			$feeds2 .= "<tr class='tr-icon'><td class='hover-callback' width='16px'>$scheduled_callback</td><td class='hover-ishuman' width='16px'>$human_answered</td><td><input type='checkbox' value='$row[status]' id='$row[status]'><label style='display:inline;' for='$row[status]'> $row[status_name]</label></td></tr>";	
		} else {
			$feeds1 .= "<tr class='tr-icon'><td class='hover-callback' width='16px'>$scheduled_callback</td><td class='hover-ishuman' width='16px'>$human_answered</td><td><input type='checkbox' value='$row[status]' id='$row[status]'><label style='display:inline;' for='$row[status]'> $row[status_name]</label></td></tr>";
		}
	}
	if(!mysql_num_rows($query)){
...
// BASES DE DADOS 
	...
	if(mysql_num_rows($query) > 1){
		...
		} else { $bd_list = "Sem Bases de Dados associadas.";
	...
	if(mysql_num_rows($query)>0){} else {
		...
		for($i=0;$i<count($default_recycle);$i++)
		{
			$recycle .= "<tr id='$default_recycle[$i]'>
							<td>
								<input type='checkbox' name='$default_recycle[$i]' id='$default_recycle[$i]' >
	...
	<div id="wizard-tabs" style="display:none"> 
		...
		<tr>
			<td class="td-half-left">
				<label class="input-text-label" for="campaign-name">Nome da Campanha</label><input class="input-text" type="text" id="campaign-name">
				<label class="input-text-label" for="campaign-description">Descrição da Campanha</label><textarea class="input-textarea" id="campaign-description"></textarea>	
			</td>
...
function submit_OpcoesGerais(){
	...
	if($("#uniform-lead_order_random span").hasClass("checked")){ var campaign_lead_order = "RANDOM"; } else { var campaign_lead_order = "DOWN"; }
	...
	if(campaign_name=="" || campaign_name==null){ return false; }
	else {
		$.ajax({
			type: "POST",
			url: "_requests.php",
...
$(" #new_pause ").click(function(){
	$.ajax({
		...
		/*		if($( "#span-pauses-1 input" ).length > $( "#span-pauses-2 input" ).length)
				{
					$( "#span-pauses-2" ).append("<input type='checkbox' value='"+pause_id+"' name='"+pause_id+"' id='"+pause_id+"' ><label style='display:inline;' for='"+pause_id+"'>"+pause_name+"</label><br>")
				} else {
					$( "#span-pauses-1" ).append("<input type='checkbox' value='"+pause_id+"' name='"+pause_id+"' id='"+pause_id+"' ><label style='display:inline;' for='"+pause_id+"'>"+pause_name+"</label><br>")
				} */
...
$("#dialog-edit-pause-time").dialog({ 
	title: ' <span style="font-size:13px; color:black">Alterar os Tempos de Pausa</span> ',
	autoOpen: false,
	height: 300,
	width: 300,
	resizable: false,
...
function submit_Feedbacks(){
	...
	var feed_callback = new Array();
	var feed_ishuman = new Array();
	var feed_active = new Array();
	...
	$("#table-feeds-1 tr td div span").each(function(){
		...
		if($(this).hasClass("checked")){feed_active.push("Y")} else {feed_active.push("N")}
	});
...
$(" #new-feed ").click(function(){
	$.ajax({
			...
			success: function(data)	
			{
				if($( "#temp-nofeeds" ).length > 0){ $( "#td-feeds-1" ).html(""); }
				var feed_id = data;
				var feed_name = $("#feed-name").val();

				if($( "#uniform-is_human span" ).hasClass( "checked" ) ) { var human_icon = "<img class='mono-icon'  src='/images/icons/mono_speech_16.png'>"; } else { var human_icon = ""; }	
				if($( "#uniform-is_callback span" ).hasClass( "checked" ) ) { var callback_icon = "<img class='mono-icon'  src='/images/icons/mono_phone_16.png'>"; } else { var callback_icon = ""; }	
...
$('.hover-callback').live('mouseenter mouseleave', function(event) {
  if (event.type == 'mouseenter') {
    if($(this).html()==""){$(this).html("<img class='mono-icon' src='/images/icons/mono_phone_16.png'>"); hover_flag = 1; icon_flag=0;} else {hover_flag = 0; icon_flag = 1; }
								
  } else {
    if(hover_flag==1){$(this).html("")}
  }
});

$('.hover-ishuman').live('mouseenter mouseleave', function(event) {
  if (event.type == 'mouseenter') {
    if($(this).html()==""){$(this).html("<img class='mono-icon' src='/images/icons/mono_speech_16.png'>"); hover_flag = 1; icon_flag=0;} else {hover_flag = 0; icon_flag = 1; }
...
$('.hover-callback').live("click", function(){ if(icon_flag==1){$(this).html("")} else {hover_flag=0;}  });
$('.hover-ishuman').live("click", function(){ if(icon_flag==1){$(this).html("")} else {hover_flag=0;} });
...
$("#new-database").click(function(){
	...
	$.ajax({
			...
			$("#dialog-testing").html(list_id);
				...
				$("#database-list").append("<li id='"+list_id+"' class=''><table><tr><td style='width:16px'><input name='i-"+list_id+"' id='i-"+list_id+"' type='checkbox'></td><td><label class='' style='display:inline;' for='i-"+list_id+"'>"+database_name+"</label></td><td width='24px'><img class='mono-icon cursor-pointer' src='/images/icons/mono_icon_database_16.png'></td><td width='24px'><img class='mono-icon cursor-pointer' src='/images/icons/mono_wrench_16.png'></td></tr></table></li>"); 
				$("#add-database_name").val("");
...
$("#dialog-load-leads").dialog({ 
	...
	open: function(){ $("button").blur(); $("#div-field-match").hide(); $("#table-file-loader-progress").hide(); $("#table-file-converter-progress").hide(); $("#continue-button").hide(); }
});
