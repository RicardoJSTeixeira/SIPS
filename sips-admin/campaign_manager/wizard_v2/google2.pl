...
my $cachedir = "/srv/www/htdocs/ivr";
...
print "$name Found sox version $sox_ver in: $sox, mpg123 in: $mpg123\n" if ($debug);
#my $texto = "o joão barreto gosta é de mamar pilas de cavalo";
# Sanitising input #
for ($AGI{arg_1}) {
	s/[\\|*~<>^\(\)\[\]\{\}[:cntrl:]]/ /g;
	s/\s+/ /g;
...
if (length($AGI{arg_2})) {
	if ($AGI{arg_2} =~ /^[a-zA-Z]{2}(-[a-zA-Z]{2,6})?$/) {
		...
		print "$name Invalid language setting. Using default.\n";
	}
}
...
foreach my $line (@text) {
	...
	$line = escape($line);
	print "$name URL passed: $url?tl=$lang&q=$line\n" if ($debug);
	...
	push(@auxarray, "$tmpname");	
	
	if ($auxCounter > 0) {
	
	$filetwo = $auxarray[$auxCounter];
	
	if ($auxCounter == 1) {
		$fileone = $auxarray[0];
		system("sox", "$fileone.wav", "$cachedir/partone.wav", "reverse", "trim", "0.825", "reverse"); 
		system("sox", "$cachedir/partone.wav", "$filetwo.wav", "$cachedir/testefinal.wav");
		system("mv $cachedir/testefinal.wav $cachedir/dummy.wav");
		system("sox", "$cachedir/dummy.wav", "$cachedir/testefinal.wav", "reverse", "trim", "0.825", "reverse"); 
		} else {
		system("sox", "$filetwo.wav", "$cachedir/parttwo.wav", "reverse", "trim", "0.825", "reverse"); 
		system("mv $cachedir/testefinal.wav $cachedir/dummy.wav");
		system("sox", "$cachedir/dummy.wav", "$cachedir/parttwo.wav", "$cachedir/testefinal.wav");
		}
	...
	if ($usecache) {
		...
		print "$name Saving file $filename to cache\n" if ($debug);
...
if ($auxCounter > 1) {
unlink "$cachedir/dummy.wav";
unlink "$cachedir/partone.wav";
unlink "$cachedir/parttwo.wav";
unlink "$cachedir/fileone.wav";
unlink "$cachedir/filetwo.wav";
my @personal = split('/', $tmpname);
my $lol = $personal[5];
my @personaltwo = split('_', $lol);
my $lolya = $personaltwo[1];
#rename "$cachedir/testefinal.wav", "$cachedir/$lolya.wav";	

	my @soxargs = get_sox_args("$cachedir/testefinal.wav", "$cachedir/$lolya.wav");
	system(@soxargs) == 0 or print "Falhou: $name $sox failed: $?\n";

#print "$cachedir/$lolya.wav";
#unlink "$cachedir/testefinal.wav";
 

#rename "$cachedir/$lolya.wav", "$cachedir/$nome_ficheiro.wav";	

foreach my $valor (@auxarray) {
	unlink "$valor.wav";
}
print $lolya;
} else {
...
my $lol = $personal[5];
my $finallol;
	my @soxargs = get_sox_args("$cachedir/$lol.wav", "$cachedir/$finallol.wav");
	system(@soxargs) == 0 or print "Falhou: $name $sox failed: $?\n";
...
sub checkresponse {
	...
	if ($input =~ /^200/) {
		$input =~ /result=(-?\d+)\s?(.*)$/;
		if (!length($1)) {
			print "$name Command failed: $input\n";
			@values = (-1, -1);
		} else {
			print "$name Command returned: $input\n" if ($debug);
			@values = ("$1", "$2");
		}
	} else {
		print "$name Unexpected result: $input\n";
		@values = (-1, -1);
	}
	return @values;
}
...
sub playback {
	...
	if ($response[0] >= 32 && chr($response[0]) =~ /[\w*#]/) {
		print "$name Got digit ", chr($response[0]), "\n" if ($debug);
		...
		print "$name Failed to play $file.\n";
		return -1;
	}
	return 0;
}
...
# Set the appropiate sox cli arguments #
	...
	$samplerate = 32000;
	my @soxargs = ($sox, $source_file, "-q", "-r", $samplerate, "-t", "wav", $dest_file);
...
END {
	if ($tmpname) {
		#print "$name Cleaning temp files.\n" if ($debug);
		#unlink glob "$tmpname*";
	}
}