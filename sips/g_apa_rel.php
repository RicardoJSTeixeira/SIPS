...
if ($action=="result") {
	$filename = "/var/www/html/goautodial/goreload.txt";
...
if ($action=="check") {
	echo "<br><FONT FACE='VERDANA' COLOR=BLACK SIZE=2><strong>Apache Webserver is now being reloaded!</strong><br />\n";
}

//sleep(20);
if ($action=="restart") {
	$startme = shell_exec('/usr/share/goautodial/goautodialc.pl "/usr/bin/nohup /etc/init.d/httpd reload"');
	$filename = "/var/www/html/goautodial/goreload.txt";
