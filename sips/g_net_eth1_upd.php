...
if ((!$new_ip)||(!$net_mask)||(!validateIpAddress($new_ip))||(!validateIpAddress($net_mask)))
	{
	  	echo "<br><br><FONT style='font-family: Verdana, Helvetica, sans-serif; font-size: 12px;'   COLOR=BLACK SIZE=2>Error changing eth1 IP configuration!<br />\n";
		echo "<FONT style='font-family: Verdana, Helvetica, sans-serif; font-size: 12px;'   COLOR=BLACK SIZE=2>Please type-in a valid eth1 new ip and netmask!<br />\n"; 
	}
	else
	{	

#exec('sudo -u root /bin/cp -f /etc/sysconfig/network-scripts/ifcfg-eth1 /etc/sysconfig/network-scripts/ifcfg-eth1.bak.gad');		
...
if (isset($_POST['submit'])) 
 {
   if ($_POST['submit'] == "Save") 
	  ...
	  $lineme=count_words(); 
      $file = fopen("/etc/sysconfig/network-scripts/ifcfg-eth1", "w") or exit("Unable to open file!");
      if ($file != FALSE) {
	  $o=0;
	  while($lineme > $o)
  		{
	  		...
	  		system("/bin/echo '$datar' >> /etc/sysconfig/network-scripts/ifcfg-eth1");
	  		$o++;
  		}
	echo "<br><br><FONT style='font-family: Verdana, Helvetica, sans-serif; font-size: 12px;'   COLOR=BLACK SIZE=2>eth1 IP configuration changes was successfully saved!<br />\n"; 		  		
	...
	{
	  	echo "<br><br><FONT style='font-family: Verdana, Helvetica, sans-serif; font-size: 12px;'   COLOR=BLACK SIZE=2>Error saving eth1 IP configuration!<br />\n";
