...
switch($stage) {						
	case "EDIT":
		$stmt="UPDATE go_recording_access SET ip_block_list='".$ip_block_list."',folder='".$folder."',folder_alias='".$folder_alias."',active='".$status."' WHERE access_id='".$access_id."'";
	...
	case "ADD":
		...
		$stmt2="SELECT * FROM go_recording_access where ip_address='".$ip_address."' ";
		...
		<?
		  ...
		  $stmt="INSERT INTO go_recording_access VALUES ('','".$ip_address."','".$ip_block_list."','".$folder."','".$folder_alias."','".$status."')";
		  $rslt=mysql_query($stmt, $linkd);
		  exec ('/usr/share/goautodial/goautodialc.pl "/usr/share/goautodial/go_httpd_conf.pl"');
		  break;
		}
	...
	case "DELETE":
		$stmt="DELETE FROM go_recording_access WHERE access_id='".$access_id."'";
		$rslt=mysql_query($stmt, $linkd);
		exec ('/usr/share/goautodial/goautodialc.pl "/usr/share/goautodial/go_httpd_conf.pl"');
		break;
...
function fnValidateIPAddress(ipaddr) {
	...
	errorString = "Invalid Format IP Address"; 
	alert(errorString);
        return false;
    }
}
...
<br>
<table>
	<tr><td class='myfontb'>&raquo;&nbsp;GO&nbsp;HTTPD&nbsp: RECORDING ACCESS</td></tr>
</table>  	
<br> 
...
<?php
$stmt='SELECT * FROM go_recording_access order by active desc';
$rslt=mysql_query($stmt, $linkd);
